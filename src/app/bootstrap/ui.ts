...
import type { ContentId, ResourceId, ProviderReference } from "../../contracts/content.ts";
...
export class ApplicationSeatUi implements ApplicationInputUi {
  ...
  private nativeInventoryItem: { readonly item: ItemId; readonly source: ProviderReference; readonly value: NonNullable<NativeQ2HudArsenal["selectedItem"]> } | null = null;
  ...
  private async nativeSourceLocalizer(content: ContentId, assets: ApplicationAssets): Promise<(text: string, args?: readonly string[]) => string> {
    if (this.nativeHudLocalizer !== undefined) return this.nativeHudLocalizer(content);
    this.nativeHudLocalizations ??= new ApplicationRereleasePresentation(assets, [{ seat: this.local.player.seat.id, actor: this.local.player.actor }], this.nativeHudSettings());
    return this.nativeHudLocalizations.sourceLocalizer(this.local.player.seat.id, content);
  }

  private async prepareNativeInventory(assets: ApplicationAssets): Promise<void> {
    this.nativeInventoryItem = null;
    const inventory = this.simulation.playerUi(this.local.player.actor).nativeInventory, presentation = inventory?.presentation;
    if (inventory === undefined || presentation === undefined) return;
    ...
    this.nativeInventoryItem = { item: item.item, source: presentation.source, value: { label: item.label, localizedLabel,
      icon: resource === null ? null : { resource, aspect: this.weaponAssets.aspect(resource) } } };
  }
  ...
  async prepareNativeQ2Hud(frame: NativeQ2HudFrame, content: ContentId, assets: ApplicationAssets, context: UiDrawContext,
    ...
    if (component === undefined) await this.prepareNativeInventory(assets);
    ...
    if (frame.protocol.kind === "q2-rerelease" || frame.protocol.kind === "q2-kex" || frame.protocol.kind === "q2-kex-demo") {
      ...
      const localize = await this.nativeSourceLocalizer(content, assets);
