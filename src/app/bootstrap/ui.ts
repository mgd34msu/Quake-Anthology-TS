...
export class ApplicationSeatUi implements ApplicationInputUi {
  ...
  constructor(readonly local: LocalInput, readonly art: NativeUiArt, input: ApplicationInput,
    ...
    this.settings = registerSettingsMenus(this.controller, [...display, ...(viewSetting === undefined ? [] : [viewSetting]), ...images, ...(language === undefined ? [] : [language]), bindingMenu, { id: "ui:settings:gyro", label: "Gyro controls", kind: "button", category: "input", enabled: () => true, activate: () => { this.controller.openMenu(gyro.root); } }, ...serverMenu, ...input.inputDevices.bindings(), ...bindInputRoutingSettings(() => input.router, () => input.controllers.devices), ...((input.options.network.kind === "offline" || input.options.network.kind === "native-server" || input.options.network.kind === "q2-server") ? [
      ...
      {id:"ui:input:local-drop",label:"Remove this player",kind:"button",category:"input",enabled:()=>input.locals.length>1,activate:()=>{command("local_drop",[String(seat.index+1)]);}},
