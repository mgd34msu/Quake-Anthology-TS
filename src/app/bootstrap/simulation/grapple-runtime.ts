...
export class GrappleRuntime {
  ...
  constructor(readonly selection: Extract<GrappleSelection, { readonly kind: "enabled" }>, readonly source: GrappleSource,
    ...
    source.game.host.actors.onRelease(actor => { this.controls.delete(actor.id); return undefined; });
  ...
  release(actor: ActorId): undefined {
    ...
    if (this.hook(actor) === null) return undefined;
  ...
  step(actor: ActorId, enabled: boolean): undefined {
    const state = this.controls.get(actor);
    if (state === undefined) return undefined;
    if (!enabled || state.released) return this.release(actor);
  ...
  capture(): GrappleRuntimeCheckpoint {
    const common = { version: 1, controls: [...this.controls].map(([actor, state]) => ({ actor: saved(actor), ...state })), random: this.random.checkpoint() } satisfies Omit<GrappleRuntimeCheckpoint, "source">;
    const source = this.source;
    switch (source.kind) {
