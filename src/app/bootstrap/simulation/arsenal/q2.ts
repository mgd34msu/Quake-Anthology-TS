...
import type { Q2WeaponInput, Q2WeaponOwner } from "../../../../content/q2/foundation/weapons/types.ts";
...
export class Q2SelectedArsenal implements SelectedArsenal {
  ...
  constructor(private readonly options: Q2SelectedArsenalOptions) {
    ...
    const definitions = options.weapons.registeredDefinitions();
    if (definitions.length !== Q2_BASE_WEAPONS.length || definitions.some(definition => !Q2_BASE_WEAPONS.some(base => base.name === definition.name)))
      throw new Error("Selected Q2 arsenal supports only base weapons");
    for (const definition of Q2_BASE_WEAPONS) for (const item of definition.ammo === null ? [definition.item] : [definition.item, definition.ammo])
    ...
    if (options.loadout !== undefined) {
      if (!Q2_BASE_WEAPONS.some(definition => definition.item === options.loadout?.weapon)) throw new Error("Selected Q2 starter weapon is outside the base arsenal");
      for (const entry of options.loadout.inventory) if (!Number.isSafeInteger(entry.count) || entry.count < 0
        || !Q2_BASE_WEAPONS.some(definition => definition.item === entry.item || definition.ammo === entry.item)) throw new Error("Selected Q2 starter inventory is invalid");
    }
  }
  ...
  admit(actor: OwnedActor, _maxHealth: number): ArsenalState {
    ...
    const items = new Set(Q2_BASE_WEAPONS.flatMap(definition => definition.ammo === null ? [definition.item] : [definition.item, definition.ammo]));
    ...
    const weapon = loadout === undefined ? "blaster" : Q2_BASE_WEAPONS.find(definition => definition.item === loadout.weapon)?.name;
  ...
  read(actor: ActorId): ArsenalState {
    ...
    return { ...arsenal, ammo: arsenal.ammo.filter(entry => Q2_BASE_WEAPONS.some(definition => definition.item === entry.item || definition.ammo === entry.item)) };
  }
  select(actor: ActorId, item: ItemId): boolean {
    const definition = Q2_BASE_WEAPONS.find(definition => definition.item === item);
  ...
  handoff(actor: ActorId): PrimaryWeaponHandoff {
    ...
    return { provider: this.provider,
      accepts: item => { const definition = Q2_BASE_WEAPONS.find(definition => definition.item === item);
      ...
      resume: item => { const observation = this.options.observe(actor);
        const definition = item === null ? null : Q2_BASE_WEAPONS.find(definition => definition.item === item);
  ...
  pickupWeapons(actor: OwnedActor, items: readonly ItemId[], selection: PickupSelection): undefined {
    ...
    for (const item of items) {
      ...
      if (q1Q2PickupSelect(current, item, selection)) this.select(actor.id, item);
    }
  }
  step(input: WeaponStepInput, intent: ArsenalIntent | undefined): WeaponStepResult {
    ...
    if (intent?.weapon != null) {
      if (!Q2_BASE_WEAPONS.some(definition => definition.item === intent.weapon)) throw new Error("Weapon does not belong to selected Q2 arsenal");
