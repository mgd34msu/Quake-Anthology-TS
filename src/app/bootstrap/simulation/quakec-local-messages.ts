...
export class QuakeCLocalMessages {
  ...
  private retained(state: Map<string, NetQuakeMessage>, message: NetQuakeMessage): void {
    switch (message.kind) {
      ...
      case 'name': case 'colors': case 'frags': state.set(`${message.kind}:${message.slot}`, message); break;
  ...
  presentation(actor: ActorId): readonly NetQuakeMessage[] {
    return [...(this.clients.get(actor)?.values() ?? [])].filter(message => message.kind === 'intermission' || message.kind === 'finale' || message.kind === 'cutscene' || message.kind === 'cd-track' || message.kind === 'pause');
  }
...
export function presentQuakeCLocalMessage(message: NetQuakeMessage, target: ActorId | null, state: QuakeCLocalMessages, host: QuakeCLocalMessageHost): void {
  ...
  switch (message.kind) {
    ...
    case 'nop': case 'name': case 'colors': case 'frags': case 'set-view': break;
    ...
    case 'light-style': if (target !== null) throw new Error('Client-specific QC light styles require a per-seat scene style owner'); host.emit({kind:'lightstyle',style:message.index,pattern:message.value}); break;
