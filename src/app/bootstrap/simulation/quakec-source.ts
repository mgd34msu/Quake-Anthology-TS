...
import { QcClientHost } from "../../../compat/qc/client-host.ts";
...
export class QuakeCSource {
  ...
  constructor(readonly prepared: PreparedQuakeCSource, readonly options: QuakeCSourceOptions) {
    ...
    for (const [name, value] of Object.entries({ skill: String(options.skill), deathmatch: options.mode === "deathmatch" ? "1" : "0", coop: options.mode === "coop" ? "1" : "0",
      teamplay: "0", sv_gravity: "800", sv_maxspeed: "320", samelevel: "0", timelimit: "0", fraglimit: "0", gamecfg: "0", registered: "1" })) this.cvars.register(name, value);
    ...
    const presentation = createQcPresentationBindings(this.worldHost, { content: prepared.execution.owner.content, events: options.events,
      loading: () => this.spawning, print: options.print, precache: (kind, name) => this.precache(kind, name), lookup: (kind, name) => this.precached.get(`${kind}:${name}`) ?? null });
    ...
    const host = new Map([...this.worldHost.host, ...presentation, ...movement, ...this.clients.host]);
    ...
    this.machine = new QcMachine({ program: prepared.program, entities: this.entities, numeric: createNumericOperations(Q1_DONOR_PROFILE),
      ...
      functionBoundary: damage.functionBoundary, observeCall: call => damage.observeCall(call), observeEntityStore: store => damage.observeEntityStore(store) });
