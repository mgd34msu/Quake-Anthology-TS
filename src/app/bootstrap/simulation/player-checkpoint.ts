...
export function captureMovementPlayer(player: Readonly<MovementPlayer>) {
  const intent = player.arsenalIntent;
  const arsenalIntent = intent?.impulse === undefined || intent.impulse === 0 ? null
    : { provider: intent.provider, weapon: null, useHoldable: false, impulse: intent.impulse };
  return { version: 1, bodyShapeBase: player.bodyShapeBase, fixedPoseActive: player.fixedPoseActive, arsenalIntent, netQuakeCommand: player.netQuakeCommand, actor: savedActorId(player.actor.id), clientSlot: player.client.slot, state: saveMovement(player.state), arsenal: player.arsenal, animation: player.animation,
    ...
    intermission: player.intermission, cutscene: player.cutscene, flight: player.flight, gravityMultiplier: player.gravityMultiplier, worldGravity: player.worldGravity, buttons: player.buttons, previousButtons: player.previousButtons, lastSequence: player.lastSequence, lastWeaponSeconds: player.lastWeaponSeconds };
}
export function readMovementPlayer(reader: SaveReader, reference: ActorReference) {
  reader.field("version").literal(1);
  const savedIntent = reader.field("arsenalIntent");
  const arsenalIntent = savedIntent.value === undefined ? undefined : savedIntent.nullable((value): ArsenalIntent => {
    const impulse = value.field("impulse");
    const number = impulse.value === undefined ? undefined : impulse.integer(0);
    if (number !== undefined && number > 255) impulse.fail("source impulse exceeds one byte");
    return { provider: namespaced(value.field("provider")), weapon: value.field("weapon").nullable(namespaced),
      useHoldable: value.field("useHoldable").boolean(), ...(number === undefined ? {} : { impulse: number }) };
  }) ?? undefined;
  ...
  return { bodyShapeBase: reader.field("bodyShapeBase").value === undefined ? null : reader.field("bodyShapeBase").nullable(readBounds), fixedPoseActive: reader.field("fixedPoseActive").value === undefined ? false : reader.field("fixedPoseActive").boolean(), arsenalIntent, netQuakeCommand, flight: reader.field("flight").value === undefined ? false : reader.field("flight").boolean(), actor: readSavedActor(reader.field("actor")), clientSlot: reader.field("clientSlot").integer(0), state: readMovement(reader.field("state"), reference), arsenal: readArsenal(reader.field("arsenal")), animation: readActorAnimation(reader.field("animation")),
