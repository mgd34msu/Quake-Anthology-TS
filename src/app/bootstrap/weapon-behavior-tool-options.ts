...
export type WeaponBehaviorToolCommand = { readonly action: "help" }
  | ToolContent & ({ readonly action: "inspect" } | { readonly action: "declare"; readonly id: `${string}:${string}`;
...
export const weaponBehaviorToolHelp = `Usage:
  ...
  --artifact PATH       Mounted QuakeC program (defaults to descriptor or progs.dat/qwprogs.dat)
...
export function parseWeaponBehaviorTool(argv: readonly string[]): WeaponBehaviorToolCommand {
  ...
  if ((action !== "inspect" && action !== "declare") || product === undefined || product.startsWith("--"))
    throw new Error(weaponBehaviorToolHelp);
  const flags = new Map<string, string>();
  for (let index = 2; index < argv.length; index += 2) {
    const key = argv[index], value = argv[index + 1];
    if (key === undefined || !["--content", "--user-content", "--artifact", "--id", "--title", "--role", "--fire", "--activate"].includes(key)) throw new Error(`Unknown behavior option: ${key}`);
