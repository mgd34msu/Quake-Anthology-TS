...
export function encodeUnifiedFrame(frame: UnifiedPresentationFrame): Uint8Array {
  ...
  const value = encodeCheckpointValue({schema:'qts-unified-frame',version:4,epoch:frame.epoch,acknowledgedInput:frame.acknowledgedInput,prediction:encodeUnifiedPrediction(frame.prediction),
...
export function readUnifiedFrame(bytes:Uint8Array) {
  ...
  r.field('schema').literal('qts-unified-frame');r.field('version').choice(2,3,4);
