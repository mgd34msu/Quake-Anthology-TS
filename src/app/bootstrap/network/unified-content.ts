...
export function createUnifiedComposition(recipe: ExecutableRecipe, sidecars: readonly UnifiedMapSidecar[] = []): UnifiedCompositionIdentity {
  ...
  const composition = { schemaVersion: 1, recipe: normalized, snapshotSchema: 'qts:snapshot-v6', actorConfigurations: [], sidecars: checked } satisfies UnifiedCompositionIdentity['composition'];
...
export function readUnifiedComposition(value: unknown): UnifiedCompositionIdentity {
  ...
  composition.field('schemaVersion').literal(1); composition.field('snapshotSchema').literal('qts:snapshot-v6');
