...
function readPresentationOwner(reader: SaveReader): PresentationOwner {
  ...
  if (!Number.isSafeInteger(generation)) return reader.fail("Invalid presentation generation");
...
function readEventSimulationPresentationEvent(reader: SaveReader, identity: UnifiedIdentityDecoder): EventSimulationPresentationEvent {
  const event = readSourcePresentationEvent(reader, identity), owner = reader.field("owner");
  return owner.value === undefined ? event : { ...event, owner: readPresentationOwner(owner) };
}
function readSourcePresentationEvent(reader: SaveReader, identity: UnifiedIdentityDecoder): EventSimulationPresentationEvent { void identity; switch (reader.field('kind').string()) {
case "debug-graph": return { kind: "debug-graph", event: { kind: reader.field("event").field("kind").literal("sample"), value: reader.field("event").field("value").finite(), color: reader.field("event").field("color").integer() }, content: readContentId(reader.field("content"), identity), sequence: reader.field("sequence").integer(0), seconds: reader.field("seconds").finite(), ...(reader.field("recipient").value === undefined ? {} : { recipient: readActorId(reader.field("recipient"), identity) }) };
case "presentation-owner": return { kind: "presentation-owner", event: { kind: reader.field("event").field("kind").choice("retired", "refreshed"), owner: readPresentationOwner(reader.field("event").field("owner")) }, content: readContentId(reader.field("content"), identity), sequence: reader.field("sequence").integer(0), seconds: reader.field("seconds").finite() };
case "q1": return ({ "kind": reader.field("kind").literal("q1"), "event": readEventQ1Event(reader.field("event"), identity), "sequence": reader.field("sequence").finite(), "content": readContentId(reader.field("content"), identity), "seconds": reader.field("seconds").finite(), ...(reader.field("sourceEntity").value === undefined ? {} : { "sourceEntity": readEventSimulationPresentationEventQ1SourceEntity(reader.field("sourceEntity"), identity) }), ...(reader.field("recipient").value === undefined ? {} : { recipient: readActorId(reader.field("recipient"), identity) }) });
...
function writeEventSimulationPresentationEvent(value: EventSimulationPresentationEvent): unknown {
  return { ...writeSourcePresentationEvent(value), ...(value.owner === undefined ? {} : { owner: value.owner }) };
}
function writeSourcePresentationEvent(value: EventSimulationPresentationEvent) { switch (value.kind) {
case "debug-graph": return { kind: value.kind, event: value.event, content: value.content, sequence: value.sequence, seconds: value.seconds, ...(value.recipient === undefined ? {} : { recipient: writeActorId(value.recipient) }) };
case "presentation-owner": return { kind: value.kind, event: value.event, content: value.content, sequence: value.sequence, seconds: value.seconds };
case "q1": return ({ "kind": value["kind"], "event": writeEventQ1Event(value["event"]), "sequence": value["sequence"], "content": writeContentId(value["content"]), "seconds": value["seconds"], ...(value["sourceEntity"] === undefined ? {} : { "sourceEntity": writeEventSimulationPresentationEventQ1SourceEntity(value["sourceEntity"]) }), ...(value.recipient === undefined ? {} : { recipient: writeActorId(value.recipient) }) });
