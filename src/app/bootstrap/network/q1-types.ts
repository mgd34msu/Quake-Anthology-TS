...
export interface Q1ApplicationPlayer { readonly client: ClientId; readonly actor: ActorId; readonly sourceEntity: number; }
export type Q1ApplicationMessage = Exclude<NetQuakeMessage, {kind: 'entity'}>;
export interface Q1ApplicationGameState { readonly info: Extract<NetQuakeMessage, {kind: 'server-info'}>; readonly baselines: ReadonlyMap<number, Q1ExtendedEntityState>; readonly signon: readonly Q1ApplicationMessage[]; }
export interface Q1ApplicationServerHost {
 readonly protocol: Q1ProtocolIdentity;
 readonly maxClients: number;
 readonly mapName: string;
 supportsSourceWire(): WireAdmission;
 admit(from: NetworkAddress): {readonly kind: 'accepted'; readonly player: Q1ApplicationPlayer} | {readonly kind: 'rejected'; readonly reason: string};
 ...
 frame(player: Q1ApplicationPlayer, output: SimulationOutput): {readonly seconds: number; readonly messages: readonly Q1ApplicationMessage[]; readonly entities: readonly Q1ExtendedEntityState[]};
...
export interface Q1ServerNetworkOptions<TAddress extends NetworkAddress> { readonly transport: DatagramTransport<TAddress>; readonly host: Q1ApplicationServerHost; readonly timeoutMilliseconds?: number; }
