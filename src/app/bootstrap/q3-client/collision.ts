...
export function q3ClientCollision(queries: SceneQueries, settings: CollisionMapSettings): CollisionWorld {
  ...
  const trace = (query: TraceQuery, origin: Vec3, angles: Vec3): TraceResult => {
    ...
    const result = queries.trace({ start: query.start, end: query.end,
      ...
      target: target(query.modelIndex, origin, angles), policy: { kind: "q3", contentsMask: query.mask, curves: !settings.noCurves && (query.curves ?? true), playerCurveClip: settings.playerCurveClip && (query.playerCurveClip ?? true) }, numeric, passActor: null });
  ...
  const contents = (point: Vec3, model: number | undefined, origin: Vec3, angles: Vec3): number => {
    ...
    const result = queries.pointContents({ point, target: target(model, origin, angles),
      policy: { kind: "q3", contentsMask: -1, curves: !settings.noCurves, playerCurveClip: settings.playerCurveClip }, numeric, passActor: null });
