...
interface ApplicationCommandRequest { readonly target: "application" | "client"; readonly name: string; readonly arguments_: readonly string[]; readonly seat: SeatId | null; readonly source?: CommandContext; }
...
export class Application {
  ...
  static async open(options: ApplicationOptions, host: ApplicationHost, recipe?: ExecutableRecipe, preferences?: FrontendPreferenceOverrides, initialSave?: SaveImage): Promise<Application> {
    ...
    try {
      ...
      if (savedSettings !== null) {
        for (const slot of savedSettings.clientSlots) restoredClients.set(slot, session.createClient(slot));
        if (!options.dedicated) for (const client of restoredClients.values()) {
      ...
      if (application !== null) { if (initialSave !== undefined) application.simulation.q3Guest()?.discard(); await application.close(); }
  ...
  private inputActions(simulation = this.simulation, content = this.content, q2Console = this.q2Console, localGuest = this.localGuest): ApplicationInputCommands {
    return { ...(this.host.llm === undefined ? {} : { llm: this.host.llm }), quit: () => { if (localGuest !== null && localGuest !== this.localGuest) throw new Error("Guest candidate requested quit"); return this.requestQuit(); },
      ...
      }, seat: id => localGuest?.seats.get(id)?.cvars ?? this.graphical?.q3.get(id)?.client.cvars ?? null },
  ...
  private async bindSourceCommands(restoring = false): Promise<void> {
    const prepared = await this.prepareSourceCommands(this.simulation, this.content, restoring);
  ...
  private async openGraphical(restoring = false): Promise<void> {
    ...
    try {
      ...
      if (restoring) for (const seat of this.localSeats.values()) {
        if (this.localGuest !== null) players.push({ seat, actor: this.prepareRestoredGuestSeat(this.localGuest, this.simulation, seat, this.options) });
        else {
      ...
      for (const local of input.locals) {
        ...
        const presentation = new WorldSeatPresentation(local, assets, native, this.simulation, this.options.seats, font, characters, ui, worldEffects, sourceClient?.client ?? null, rerelease, () => this.imageSettings?.cvars.variableValue("gl_debug_distfrac") ?? 0.004, () => this.viewSettings.fieldOfView, { lines: () => this.simulation.debugLines(), lineWidth: () => this.imageSettings?.debugLineWidth ?? 2 });
  ...
  private prepareRestoredGuestSeat(world: LocalQ3GuestWorld, simulation: SharedSimulation, seat: SessionSeat,
    options: ApplicationOptions, previous?: CvarRegistry): ActorId {
    ...
    const cvars = new CvarRegistry({ dialect: "q3", context: { session: this.session.session,
      origin: { kind: "local-seat", seat: seat.id, client: seat.client.id } }, print: text => this.host.print(text),
      cheatsAllowed: () => guest.state.cvars.variableValue("sv_cheats") === 1 });
    ...
    for (const setting of previous?.snapshots() ?? [])
      if ((setting.flags & CvarFlag.UserInfo) === 0) cvars.set(setting.name, setting.value, true);
  ...
  private async createQ3SeatClient(local: LocalInput, assets: ApplicationAssets, audio: ApplicationAudio, input: ApplicationInput,
    renderer: NativeRenderer, simulation: SharedSimulation, settings?: readonly CvarSnapshot[], localGuest = this.localGuest, browser = this.guestBrowser): Promise<Q3SeatClient | null> {
    ...
    const client = await ApplicationQ3Client.create({ weaponHud: () => { const ui = simulation.playerUi(local.player.actor); return { status: ui.weaponStatus, warning: ui.arsenalWarning }; }, assets, queries: simulation.scene, initial, local, audio, movement: prediction,
      ...
      now: () => performance.now(), commands: {
        reliable: text => { const [name, ...args] = tokenizeCommand(text, "q3").argv; if (name !== undefined)
          this.requestedCommands.push({ target: "client", name, arguments_: args, seat: local.player.seat.id }); },
        console: text => input.commands.append(text, { session: this.session.session, origin: { kind: "script", name: "q3-cgame", caller: { kind: "local-seat", seat: local.player.seat.id, client: local.player.seat.client.id } } }),
        print: text => { this.host.print(text); local.console.print(text); },
      } });
    input.registerClientCommands([...client.commandNames]);
    ...
    try {
      ...
      if (save !== undefined && restoredGuest !== null) {
        if (previous !== null) {
          ...
          for (const local of previous.input.locals) {
            ...
            admissions.set(seat.client.id.slot, this.prepareRestoredGuestSeat(nextLocalGuest, nextSimulation, seat, options,
              previous.q3.get(seat.id)?.client.cvars));
          }
      ...
      if (previous !== null) {
        ...
        const input = await ApplicationInput.open(previous.renderer.window, players, options, movementDialect(options, simulation.recipe), simulation,
          this.inputActions(simulation, content, preparedCommands.q2Console, nextLocalGuest), () => performance.now(), this.inputConfig, undefined, previous.input);
        nextInput = input;
        ...
        for (const [index, local] of input.locals.entries()) {
          const sourceClient = await this.createQ3SeatClient(local, worldAssets, audio, input, previous.renderer, current, cgameSettings.get(local.player.seat.id), nextLocalGuest, nextGuestBrowser);
          ...
          const presentation = new WorldSeatPresentation(local, worldAssets, previous.renderer, current, options.seats, font, characters, ui, effects, sourceClient?.client ?? null, rerelease, () => this.imageSettings?.cvars.variableValue("gl_debug_distfrac") ?? 0.004, () => this.viewSettings.fieldOfView, { lines: () => current.debugLines(), lineWidth: () => this.imageSettings?.debugLineWidth ?? 2 });
