...
import type { LocomotionAnimation, Q1MovementState, QwMovementState } from "../../contracts/movement.ts";
...
export function finishMovement(context: MovementContext, state: Q1MovementState | QwMovementState, viewAngles: Vec3,
  ground: TraceHit, waterLevel: number, waterType: number) {
  ...
  const weapon = context.services.weaponStep({ actor: input.actor, command: input.command, frame: input.frame,
    arsenal: input.arsenal, animation: input.animation, environment: input.environment, gauntletHit: false });
  ...
  return { actor: input.actor.id, commandSequence: input.commandSequence, bounds: context.bounds, viewAngles,
