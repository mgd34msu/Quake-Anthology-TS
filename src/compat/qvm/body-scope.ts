...
export function qualifyQvmBodyCalls(image: QvmImage, body: QvmScenePresentation["body"]): ReadonlyMap<number, QvmBodyPart> {
  ...
  if ([player.centityArgument, mesh.entityArgument, mesh.stateArgument].some(argument => !Number.isInteger(argument) || argument < 0 || argument >= QVM_MAX_PRIVATE_ARGUMENT_WORDS)
