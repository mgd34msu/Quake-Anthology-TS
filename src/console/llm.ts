...
export function registerLlmCommands(commands: CommandBuffer, print: (text: string, source: CommandContext) => void): () => void {
  ...
  for (const definition of [
    { name: "llm_ask", usage: "llm_ask <question>",
      summary: "Placeholder: LLM integration is not configured. Future: print an LLM answer in the console.",
      examples: ['llm_ask "How do I change brightness?"'] },
    { name: "llm_exec", usage: "llm_exec <request>",
      summary: "Placeholder: LLM integration is not configured. Future: a predefined LLM returns valid semicolon-separated console commands, executed through the normal parser in the invoking user context. Print the exact commands executed, then their results, for inspection, copying and reuse.",
