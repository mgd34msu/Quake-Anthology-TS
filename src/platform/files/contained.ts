...
export function containedFileParts(name: string): readonly string[] {
  ...
  if (name.length === 0 || name.includes("\\") || name.includes("\0") || name.includes(":" ) || parts.some(part => part.length === 0 || part === "." || part === "..")) throw new RangeError("File path must be relative and stay inside its storage directory");
  return parts;
}
...
export function openContainedParent(root: string, name: string, create: boolean): { readonly descriptor: number; readonly leaf: string } {
  if (process.platform !== "linux") throw new Error("Contained file storage currently requires Linux directory descriptors");
  const parts = containedFileParts(name), leaf = parts.at(-1);
  if (leaf === undefined) throw new RangeError("File path has no filename");
