...
export class Q2EntityServices implements Q2GameServices {
  ...
  protected readonly sourceSlots = new Map<ActorId, number>();
  private readonly freedSlots = new Map<number, number>();
  private sequence = 0;
  currentActor: ActorId | null = null;
  protected readonly unsupported = new Set<Q2Entity>();
  ...
  attachPlayer(actor: OwnedActor): Q2Entity {
    ...
    const entity = this.attach(actor, { classname: "player", ordinal: -1, values: new Map<string, string>() });
  ...
  attach(actor: OwnedActor, fields: Q2SpawnFields): Q2Entity {
    this.host.actors.assertOwned(actor);
    if (this.entities.has(actor.id)) throw new Error("Q2 actor already has an entity continuation");
    if (this.host.bodies.read(actor.id) === null) throw new Error("Q2 entity attachment requires an existing shared body");
