...
export function throwQ2Debris(self: Q2Entity, game: Q2GameServices, model: string, speed: number, origin: Vec3): Q2Entity {
  ...
  chunk.die = (entity, services) => services.remove(entity);
  game.motion(chunk, "bounce"); game.solid(chunk, "none"); game.show(chunk);
  game.schedule(chunk, 5 + game.host.random() * 5, (entity, services) => services.remove(entity));
  return chunk;
}
...
function explosive(entity: Q2Entity, game: Q2GameServices): undefined {
  ...
  if ((entity.spawnflags & 1) !== 0) {
    ...
    entity.use = (self, services) => { self.visible = true; self.use = null; services.solid(self, "brush"); killQ2Box(self, services); return services.show(self); };
  } else if (entity.targetname !== "") entity.use = (self, services, other) => breakApart(self, services, self.actor.id, other);
  if ((entity.spawnflags & 1) !== 0 || entity.targetname === "") {
    ...
    entity.die = (self, services, reaction) => breakApart(self, services, reaction.inflictor, reaction.attacker);
  }
  game.solid(entity, entity.visible ? "brush" : "none"); game.show(entity);
  return undefined;
}
...
function barrel(entity: Q2Entity, game: Q2GameServices): undefined {
  ...
  entity.touch = (self, services, contact) => {
    ...
    const end = add(body.origin, scale(direction, 20 * state.mass / mass * services.host.frameSeconds()));
  ...
  game.schedule(entity, 2 * game.host.frameSeconds(), (self, services) => {
  ...
  return { spawn(entity, game) {
    switch (entity.classname) {
      ...
      case "misc_banner": {
        ...
        const animate: Q2Think = (self, services) => { self.frame = (self.frame + 1) % 16; services.show(self); return services.schedule(self, 0.1, animate); };
        game.show(entity); game.link(entity); game.schedule(entity, 0.1, animate); return true;
      }
      case "misc_satellite_dish": {
        ...
        const animate: Q2Think = (self, services) => {
          self.frame++; services.show(self);
          if (self.frame < 38) services.schedule(self, 0.1, animate);
          return undefined;
        };
        entity.use = (self, services) => { self.frame = 0; return services.schedule(self, 0.1, animate); };
