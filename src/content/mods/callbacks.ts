...
export function readQcCombat(reader: SaveReader): NonNullable<ModCallbackDeclaration["combat"]> {
  ...
  return { damage: sourceCall(reader.field("damage")),
    ...(scale.value === undefined ? {} : { damageScale: { function: scale.field("function").string(), entry: scale.field("entry").integer(0), exit: scale.field("exit").integer(0), damage: scale.field("damage").integer(28),
