...
import { SceneTextureLoader } from "./textures.ts";
...
export class SceneShaderRegistry {
  ...
  register(name: string, lightmap: RendererImage | null = null, lightmapIndex = -1): Promise<CompiledMaterial> {
    const key = `${this.key(name)}\0${lightmapIndex}\0${lightmap?.ordinal ?? -1}`;
    const previous = this.compiled.get(key);
    if (previous !== undefined) return previous;
    const pending = this.compile(name, lightmap, lightmapIndex);
  ...
  private async compile(name: string, lightmap: RendererImage | null, lightmapIndex: number): Promise<CompiledMaterial> {
    ...
    if (program !== undefined) {
      const result = await program.register({ whiteImage: registered(this.textures.white.image), defaultImage: registered(this.textures.missing.image),
        ...
        findImage: async request => {
          const texture = await this.textures.load(request.name, { mipmap: request.mipmap, wrap: request.wrap });
    ...
    const loaded = await this.textures.load(name, { mipmap: lightmapIndex !== -4, wrap: lightmapIndex === -4 ? "clamp" : "repeat" }), texture = loaded ?? this.textures.missing;
    const baseImage = { kind: "loaded", tmu: 0, binding: { kind: "images", playback: { kind: "single", image: { image: texture.image } } } } satisfies Parameters<typeof compileImplicitMaterial>[0]["baseImage"];
    if (loaded === null) {
      this.warnings.push(`${name}: missing shader image, using the source default material`);
      return compileImplicitMaterial({ kind: "default", name, baseImage, profile: this.profile });
    }
    if (lightmap !== null) return compileImplicitMaterial({ kind: "lightmap", name, baseImage, lightmapIndex, profile: this.profile,
      lightmapImage: { kind: "loaded", tmu: 1, binding: { kind: "images", playback: { kind: "single", image: { image: lightmap } } } } });
    if (lightmapIndex === -2) return compileImplicitMaterial({ kind: "white", name, baseImage, profile: this.profile,
      whiteImage: { kind: "loaded", tmu: 0, binding: { kind: "images", playback: { kind: "single", image: { image: this.textures.white.image } } } } });
    return compileImplicitMaterial({ kind: lightmapIndex === -4 ? "picture" : lightmapIndex === -3 ? "vertex" : "dynamic", name, baseImage, profile: this.profile });
  }
}
