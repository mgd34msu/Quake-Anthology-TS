...
export function quakeWorldChecksum(bytes: Uint8Array, sequence: number): number {
    if (!Number.isSafeInteger(sequence) || sequence < 0)
        throw new RangeError('Invalid sequence');
    const count = Math.min(60, bytes.length), offset = sequence % (chktbl.length - 8), data = new Uint8Array(count + 4);
    data.set(bytes.subarray(0, count));
    data[count] = (sequence & 255) ^ (chktbl[offset] ?? 0);
    data[count + 1] = chktbl[offset + 1] ?? 0;
    data[count + 2] = ((sequence >>> 8) & 255) ^ (chktbl[offset + 2] ?? 0);
    data[count + 3] = chktbl[offset + 3] ?? 0;
    let crc = 65535;
