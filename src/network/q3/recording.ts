...
export function q3DemoGamestate(client: Q3ClientConnection): { readonly sequence: number; readonly message: Uint8Array } {
  const sequence = (client.serverMessageSequence - 1) | 0;
  const message = encodeServerMessage(client.reliable.sequence, [client.copyGamestate()], { product: client.product, messageNumber: sequence,
...
export class Q3DemoRecording {
  ...
  constructor(readonly client: Q3ClientConnection, private readonly file: Q3DemoSink) {
    ...
    const seed = q3DemoGamestate(client);
    this.write(seed.sequence, seed.message);
  }
