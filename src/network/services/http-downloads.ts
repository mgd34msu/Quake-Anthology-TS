...
import type { DownloadExpectation, ProtocolDownloadExpectation } from './downloads.ts';
...
async function openResponse(url: URL, signal: AbortSignal): Promise<{ readonly headers: Headers; readonly body: ReadableStream<unknown> }> {
  ...
  if (response.status !== 200 || !isReadableStream(body)) {
...
export class HttpDownloadQueue {
  ...
  private async transfer(entry: Entry): Promise<HttpDownloadResult> {
    ...
    try {
      current();
      const response = await openResponse(request.url, entry.abort.signal);
      ...
      const total = length !== null && /^\d+$/.test(length) && Number.isSafeInteger(Number(length)) ? Number(length) : null;
      ...
      return { kind: published ? 'failed' : 'fallback', reason: failure(error) };
    }
  }
  cancel(): void {
    if (this.closed) return;
