...
export class ModClientOutputs {
  ...
  read(actor: ActorId): ModClientMovementOutputs | null {
    ...
    const view = this.owners.get("view-offset")?.values.get(actor), mode = this.owners.get("movement-mode")?.values.get(actor), stance = this.owners.get("stance")?.values.get(actor);
    if (view === undefined && mode === undefined && stance === undefined || !this.live(actor)) return null;
