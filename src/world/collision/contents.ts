...
export function adaptTraceResult(result: TraceResult, policy: TracePolicy): TraceResult {
    ...
    switch (policy.kind) {
        ...
        case 'q2': {
            const sky = result.kind === 'q1' && (nativeContents === -6 || ((result.surfaceFlags ?? 0) & 4) !== 0);
            // Q2 ground friction requires a surface, including ordinary Q1 solid faces.
            const surface = result.kind === 'q3'
                ? { name: '', flags: convertSurfaceFlags(result.surfaceFlags, 'q3', 'q2'), value: 0, material: '' }
                : sky ? { name: 'sky', flags: 4, value: 0, material: '' }
                : result.hit.kind !== 'none' ? { name: '', flags: 0, value: 0, material: '' } : null;
            return { ...fields, kind: 'q2', contents, surface, sourcePlane, secondary: null };
        }
