...
export function readQ2MissionPackMonstersCheckpoint(reader: SaveReader): Q2MissionPackMonstersCheckpoint {
  return { version: reader.field("version").literal(1), flyerNextMove: reader.field("flyerNextMove").choice("none", "run"), hints: reader.field("hints").nullable(decodeQ2RogueHintsCheckpoint), widowShotsFired: reader.field("widowShotsFired").number(), widowDamageMultiplier: reader.field("widowDamageMultiplier").choice(1, 2, 4), actors: reader.field("actors").list(value => { const state = value.field("state"); return { actor: readSavedActor(value.field("actor")), state: { blocked: state.field("blocked").boolean(), turretOrientation: state.field("turretOrientation").number(), healer: state.field("healer").nullable(readSavedActor), badMedic1: state.field("badMedic1").nullable(readSavedActor), badMedic2: state.field("badMedic2").nullable(readSavedActor), medicTries: state.field("medicTries").number(), summonStrength: state.field("summonStrength").number(), lastPlayerEnemy: state.field("lastPlayerEnemy").nullable(readSavedActor), badArea: state.field("badArea").nullable(readSavedActor), goodGuy: state.field("goodGuy").boolean(), widowQuadUntil: state.field("widowQuadUntil").number(), widowDoubleUntil: state.field("widowDoubleUntil").number(), widowInvulnerableUntil: state.field("widowInvulnerableUntil").number() } }; }) };
}
