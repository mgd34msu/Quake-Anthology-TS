...
function entity(reader: SaveReader): Q1SavedEntity {
  ...
  return { actor: readSavedActor(reader.field("actor")), sourceSlot: reader.field("sourceSlot").integer(0), classname: reader.field("classname").string(), sourceOrdinal: reader.field("sourceOrdinal").nullable(value => value.integer()), state: sourceState(reader.field("state")),
    ...
    move: reader.field("move").nullable(value => ({ destination: readVector(value.field("destination")), speed: value.field("speed").number(), remaining: value.field("remaining").number(), done: value.field("done").string() })),
...
function player(reader: SaveReader): Q1SavedPlayer {
  ...
  return { actor: readSavedActor(reader.field("actor")), state: { weapon: weapon(state.field("weapon")), attackHeld: state.field("attackHeld").boolean(), jumpHeld: state.field("jumpHeld").boolean(), teleportUntil: n("teleportUntil"), attackFinished: n("attackFinished"), weaponFrame: n("weaponFrame"), weaponAnimationAt: n("weaponAnimationAt"), weaponAnimationBase: n("weaponAnimationBase"), continuousFiring: state.field("continuousFiring").boolean(), nextWeaponFrame: n("nextWeaponFrame"), lightningSoundAt: n("lightningSoundAt"), nailSide: n("nailSide"),
...
export function readQ1FoundationCheckpoint(reader: SaveReader): Q1FoundationCheckpoint {
  const n = (name: string): number => reader.field(name).number();
  const basis = reader.field("basis");
  return { format: reader.field("format").literal("q1-foundation"), version: reader.field("version").literal(1), edition: reader.field("edition").choice("classic", "rerelease"), time: n("time"), frameSeconds: n("frameSeconds"), forceRetouch: n("forceRetouch"), sequence: reader.field("sequence").integer(0), nextDynamicSlot: reader.field("nextDynamicSlot").integer(0), totalSecrets: n("totalSecrets"), foundSecrets: n("foundSecrets"), totalMonsters: n("totalMonsters"), killedMonsters: n("killedMonsters"), worldType: n("worldType"), mapName: reader.field("mapName").string(),
