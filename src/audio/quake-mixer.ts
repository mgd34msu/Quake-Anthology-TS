// Quake II channel allocation, spatialization and delayed playsounds
// adapted from snd_dma.c/snd_mix.c through its TypeScript port.
...
export class QuakeMixer {
    ...
    paused = false;
    active = true;
    constructor(readonly outputRate: number, capacity = 32, readonly outputChannels: 1 | 2 = 2) {
    ...
    private spatialize(voice: Voice): void {
        ...
        const distance = Math.max(0, length - 80) * voice.attenuation;
        const mono = this.outputChannels === 1 || voice.attenuation === 0;
        const scale = 0.5;
    ...
    private allocate(entity: number, channel: number): number | null {
        ...
        for (let i = 0; i < this.voices.length; i++) {
            ...
            if (voice !== null && channel !== 0 && voice.entity === entity && voice.channel === channel)
                return i;
    ...
    startSound(sound: PcmSound, options: QuakeStartSound): boolean {
        if (!Number.isInteger(options.channel) || options.channel < 0)
    ...
    private issue(sound: PcmSound, options: QuakeStartSound): boolean {
        ...
        const attenuation = options.attenuation * (options.attenuation === 3 ? 0.001 : 0.0005);
    ...
    stopAll(): void { this.voices.fill(null); this.loops = []; this.pending = []; this.beginOffset = 0; }
    replaceActorSound(entity: number): void {
        const index = this.voices.findIndex(voice => voice !== null && voice.entity === entity);
        if (index >= 0) this.voices[index] = null;
    }
    ...
    setLoopSounds(sounds: readonly QuakeLoopSound[]): void {
        ...
        for (const entry of sounds) {
            ...
            const voice: Voice = { prepared, entity: entry.entity, channel: 0, origin: { kind: "fixed", position: entry.origin }, volume: Math.trunc((entry.volume ?? 1) * 255),
                attenuation: (entry.attenuation ?? 1) * 0.003, left: 0, right: 0, position: this.time % prepared.length, end: this.time + prepared.length - (this.time % prepared.length), auto: true };
            this.spatialize(voice);
    ...
    mix(frames: number): Int16Array {
        ...
        for (let frame = 0; frame < frames; frame++) {
            ...
            for (const voice of this.loops)
                paint(voice);
            result[frame * 2] = clamp(left >> 8);
            result[frame * 2 + 1] = clamp(right >> 8);
            this.time++;
        }
        return result;
    }
