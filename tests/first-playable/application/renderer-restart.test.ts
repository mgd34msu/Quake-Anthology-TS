...
for (const initial of ["cpu", "gl"] satisfies readonly ("cpu" | "gl")[]) test.skipIf(process.env["QUAKE_GL_SMOKE"] !== "1")(`retained ${initial} renderer stages, discards and publishes resource replacement`, async () => {
  ...
  const renderer = NativeRenderer.open({ renderer: initial, width: 16, height: 16, hidden: true, gamma: 1.25 }, owner);
  ...
  try {
    ...
    pixels.fill(0);
    const discarded = renderer.prepareRestart(initial);
    ...
    for (const kind of [initial, initial === "cpu" ? "gl" : "cpu", initial] satisfies readonly ("cpu" | "gl")[]) {
      const old = renderer.window, pending = renderer.captureNextFrame();
      const prepared = renderer.prepareRestart(kind);
    ...
    const prepared = renderer.prepareRestart(initial); prepared.publish()();
    expect(await draw(renderer, image)).toEqual(updated);
    const failure = renderer.prepareRestart(initial);
    ...
    const last = renderer.prepareRestart(initial); last.publish()();
...
test.skipIf(process.env["QUAKE_GL_SMOKE"] !== "1")("failed GL resource replay restores the old context and renderer", async () => {
  ...
  const renderer = NativeRenderer.open({ renderer: "gl", width: 16, height: 16, hidden: true, gamma: 1 }, owner);
  ...
  try {
    ...
    expect(() => renderer.prepareRestart("gl")).toThrow("candidate upload rejected");
...
test.skipIf(process.env["QUAKE_GL_SMOKE"] !== "1")("restart preserves latest mip pixels and texture-mode ordering around later creations", () => {
  ...
  try {
    ...
    const stage = renderer.prepareRestart("gl"); stage.publish()();
