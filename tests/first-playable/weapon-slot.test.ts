...
test("slot projection retains outgoing source presentation, pending selection and stored primary ammo", () => {
  ...
  const primary: PrimaryWeaponProjection = { active: { provider: "q3:weapons", item: "q3:weapon/rocketlauncher" },
    ...
    ui: { health: 90, armor: { kind: "none" }, activeWeapon: "q3:weapon/rocketlauncher", ammo: { item: "q3:ammo/rocketlauncher", count: 17 }, inventory: [], items: [] } };
  const equipment: EquipmentWeaponProjection = { weapon: { provider: "q2:equipment/ctf-grapple", item: "q2:weapon_grapple" },
  ...
  expect(dropping.model).toBe(model); expect(dropping.active).toBe(primary.active); expect(dropping.pending).toBe(equipment.weapon);
  ...
  expect(returning.model).toBe(equipment.model); expect(returning.ui.ammo).toBeNull(); expect(returning.pending).toBe(primary.pending);
