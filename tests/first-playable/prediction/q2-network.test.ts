...
test("native Q2 predicts sent moves before server execution and reconciles using channel acknowledgements", async () => {
  ...
  const remote = new Q2RemotePresentation({ identity, session, content, protocol: { kind: "q2-classic", version: 34 },
  ...
  } finally { client.close(); await server.close(); session.close(); await content.close(); }
}, 30000);
