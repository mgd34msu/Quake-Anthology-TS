...
import { qvmClientCommonSyscall } from "../../../src/compat/qvm/client-common-syscalls.ts";
import type { QvmClientCommonServices } from "../../../src/compat/qvm/client-common-syscalls.ts";
...
function fixture() {
  ...
  const cgame: QvmClientCommonServices = { ...common, role: "cgame", commands: {
  ...
  const ui: QvmClientCommonServices = { ...common, role: "ui", commands: {
...
test("QVM cvar records reference the shared registry and retain source update rules", () => {
  ...
  expect(qvmClientCommonSyscall(f.call({ role: "cgame", code: QvmCgameImport.CG_CVAR_REGISTER }, [1024, 512, 544, 1]), f.cgame)).toBe(0);
  ...
  expect(qvmClientCommonSyscall(f.call({ role: "ui", code: QvmUiImport.UI_CVAR_UPDATE }, [1024]), f.ui)).toBe(0);
  expect(record.getInt32(12, true)).toBe(400);
  expect(f.guest.readString(1040)).toBe("400");
  expect(qvmClientCommonSyscall(f.call({ role: "ui", code: QvmUiImport.UI_CVAR_VARIABLEVALUE }, [512]), f.ui)).toBe(float32ToBits(400) | 0);
  expect(qvmClientCommonSyscall(f.call({ role: "cgame", code: QvmCgameImport.CG_CVAR_SET }, [512, 0]), f.cgame)).toBe(0);
  expect(f.cvars.variableString("g_speed")).toBe("320");
  f.guest.writeString(576, "absent", 32); f.guest.bytes[608] = 99;
  expect(qvmClientCommonSyscall(f.call({ role: "ui", code: QvmUiImport.UI_CVAR_VARIABLESTRINGBUFFER }, [576, 608, 0]), f.ui)).toBe(0);
  expect(f.guest.bytes[608]).toBe(0);
  f.cvars.set("g_speed", "x".repeat(256), true);
  expect(() => qvmClientCommonSyscall(f.call({ role: "cgame", code: QvmCgameImport.CG_CVAR_UPDATE }, [1024]), f.cgame)).toThrow("MAX_CVAR_VALUE_STRING");
});
...
test("QVM common calls preserve role, current arguments and command ownership", () => {
  ...
  expect(qvmClientCommonSyscall(argc, f.cgame)).toBe(3);
  expect(qvmClientCommonSyscall(argc, f.ui)).toBeNull();
  expect(qvmClientCommonSyscall(f.call({ role: "cgame", code: QvmCgameImport.CG_ARGS }, [512, 64], argc.commandArguments), f.cgame)).toBe(0);
  expect(f.guest.readString(512)).toBe("hello world");
  expect(qvmClientCommonSyscall(f.call({ role: "ui", code: QvmUiImport.UI_ARGV }, [9, 512, 64], ["one"]), f.ui)).toBe(0);
  expect(f.guest.readString(512)).toBe("");
  f.guest.writeString(512, "weaponselect", 64);
  qvmClientCommonSyscall(f.call({ role: "cgame", code: QvmCgameImport.CG_ADDCOMMAND }, [512]), f.cgame);
  expect(f.commands.exists("weaponselect")).toBe(true);
  qvmClientCommonSyscall(f.call({ role: "cgame", code: QvmCgameImport.CG_REMOVECOMMAND }, [512]), f.cgame);
  expect(f.commands.exists("weaponselect")).toBe(false);
  qvmClientCommonSyscall(f.call({ role: "cgame", code: QvmCgameImport.CG_SENDCLIENTCOMMAND }, [512]), f.cgame);
  expect(f.reliable).toEqual(["weaponselect"]);
  expect(qvmClientCommonSyscall(f.call({ role: "ui", code: QvmUiImport.UI_MILLISECONDS }), f.ui)).toBe(1234);
  expect(qvmClientCommonSyscall(f.call({ role: "ui", code: QvmUiImport.UI_R_CLEARSCENE }), f.ui)).toBeNull();
});
...
test("UI immediate command trap waits for the actual owner while insert and append retain order", async () => {
  ...
  qvmClientCommonSyscall(f.call({ role: "ui", code: QvmUiImport.UI_CMD_EXECUTETEXT }, [2, 512]), f.ui);
  f.guest.writeString(512, "set trap_value inserted\n", 64);
  qvmClientCommonSyscall(f.call({ role: "ui", code: QvmUiImport.UI_CMD_EXECUTETEXT }, [1, 512]), f.ui);
  let released = false;
  const gate = Promise.withResolvers<void>();
  if (f.ui.role !== "ui") throw new Error("Missing UI services");
  const services: QvmClientCommonServices = { ...f.ui, commands: { ...f.ui.commands, executeNow: async text => {
    await gate.promise; f.commands.executeNow(text); released = true;
  } } };
  const result = qvmClientCommonSyscall(f.call({ role: "ui", code: QvmUiImport.UI_CMD_EXECUTETEXT }, [0, 0]), services);
