...
test("API2023 services retain public records, ordered native messages and exact unlinked clip target", async () => {
  ...
  try {
    ...
    adapter.setConfigstring(162, "#first.md2");
    expect(adapter.modelAppearance(1).attachedModels[0]).toBe("players/male/first.md2");
    adapter.setConfigstring(162, "models/ordinary.md2");
    expect(adapter.modelAppearance(1).attachedModels[0]).toBe("players/male/second.md2");
    adapter.restoreConfigstrings(new Map([[262, "#restored-second.md2"], [162, "#restored-first.md2"]]));
    expect(adapter.modelAppearance(1).attachedModels[0]).toBe("players/male/restored-first.md2");
    adapter.restoreConfigstrings(initialStrings);
    expect(adapter.modelAppearance(1).attachedModels[0]).toBe("players/male/weapon.md2");
    memory.writeInt32(view.address("s.modelindex2"), 0); memory.writeInt32(view.address("s.skinnum"), 0);
