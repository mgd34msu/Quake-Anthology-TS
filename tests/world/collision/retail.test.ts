...
for (const fixture of fixtures) {
 ...
 test.skipIf(!existsSync(archivePath))(`${fixture.path} ${fixture.map} traces through all gameplay policies`, async () => {
  ...
  try {
   ...
   const policy = policies[2]; if (policy?.kind !== 'q3') throw new Error('Missing Q3 policy');
   ...
   let active = true, bodyReads = 0;
   const calls: string[] = [];
   scene.bindActorState(() => { bodyReads++; calls.push('body'); return active ? liveState : null; });
   let currentCollision: Parameters<typeof scene.link>[1] | null = linked.collision;
   scene.bindActorCollision(() => { calls.push('collision'); return currentCollision; });
   expect(scene.trace({ ...input, passActor: null }).hit.kind).toBe('actor');
   expect(calls).toEqual(['collision', 'body']); expect(bodyReads).toBe(1); calls.length = 0;
   expect(scene.trace({ ...input, passActor: actor }).hit.kind).not.toBe('actor');
   expect(calls).toEqual(['collision', 'collision']); calls.length = 0;
   expect(scene.traceExcluding({ ...input, passActor: null }, [actor]).hit.kind).not.toBe('actor');
   expect(calls).toEqual(['collision']); calls.length = 0;
   expect(scene.trace({ ...input, passActor: null, policy: { ...policy, contentsMask: 0 } }).hit.kind).not.toBe('actor');
   expect(bodyReads).toBe(1);
   currentCollision = { ...linked.collision, role: 'trigger' };
   expect(scene.queryActors({ min: origin, max: origin }, 'solid')).toHaveLength(0);
   expect(scene.trace({ ...input, passActor: null }).hit.kind).not.toBe('actor');
   expect(bodyReads).toBe(1);
   const collisionOwner = owner.actor(2, 0);
   currentCollision = { ...linked.collision, owner: collisionOwner };
   expect(scene.trace({ ...input, passActor: collisionOwner }).hit.kind).not.toBe('actor'); expect(bodyReads).toBe(1);
   currentCollision = null;
   expect(scene.queryActors({ min: origin, max: origin })).toHaveLength(0);
   expect(scene.trace({ ...input, passActor: null }).hit.kind).not.toBe('actor'); expect(bodyReads).toBe(1);
   currentCollision = linked.collision;
   const observed = scene.queryActors({ min: origin, max: origin })[0];
   const linkedSnapshot = scene.linkedActor(actor);
   ...
   expect(linkedSnapshot?.body.state.origin).toEqual(origin);
   expect(scene.trace({ ...input, passActor: null }).hit.kind).not.toBe('actor');
