...
test.skipIf(process.env["SDL_AUDIODRIVER"] !== "dummy")("output selection preserves queued PCM and shared playback across single-device reopen and failure", () => {
    using audio = new UnifiedAudio({ milliseconds: () => 100, random: () => 0 });
    using reference = new UnifiedAudio({ milliseconds: () => 100, random: () => 0 });
    const seat = identity.seat(0), actor = identity.actor(1, 0);
    const prepare = (engine: UnifiedAudio): MusicPlayer => {
        engine.setListeners([{ seat, actor, origin, axis, gain: 0.5, underwater: false }]);
        engine.setEffectsVolume(0.3);
        engine.play({ family: "q1", sound: asset, actor, origin: { kind: "local" }, audience: { kind: "seat", seat }, channel: 1, volume: 1, attenuation: 1 });
        engine.queueStream({ id: "movie", audience: { kind: "world" }, gain: 0.2 }, { samples: new Int16Array(4096).fill(300), channels: 1, sampleRate: 44100, sourceSample: 0, resetStream: true });
        const player = new MusicPlayer(44100, "q1");
        player.setVolume(0.4);
        player.start(new MemoryPcmStream(tone), new MemoryPcmStream(tone));
        engine.attachMusic({ id: "music", audience: { kind: "world" }, gain: 1 }, player);
        return player;
    };
    const music = prepare(audio); prepare(reference);
    audio.openDevice();
    const named = audio.outputDeviceNames()[0];
    if (named === undefined) throw new Error("Dummy output must enumerate its device");
    const frames = audio.pump(2048); reference.mix(frames);
    audio.pause(true);
    const clock = audio.sampleClock, position = music.sourcePosition, pending = audio.pendingOutput;
    expect(pending.some(value => value !== 0)).toBe(true);
    audio.detachOutput();
    expect(audio.outputState).toBe("detached"); expect(audio.pendingOutput).toEqual(pending);
    using next = new UnifiedAudio({ milliseconds: () => 100, random: () => 0 });
    next.openDevice({ deviceName: named }); next.close();
    audio.selectOutput(named);
    expect(audio.outputConfiguration?.deviceName).toBe(named);
    expect(audio.outputConfiguration?.sampleRate).toBe(44100);
    expect(audio.outputState).toBe("paused");
    expect(audio.pendingOutput).toEqual(pending);
    expect(audio.sampleClock).toBe(clock); expect(music.sourcePosition).toBe(position);
    expect(() => audio.selectOutput("nonexistent-quake-audio-output")).toThrow();
    expect(audio.outputConfiguration?.deviceName).toBe(named);
    expect(audio.outputState).toBe("paused"); expect(audio.pendingOutput).toEqual(pending);
    expect(audio.sampleClock).toBe(clock); expect(music.sourcePosition).toBe(position);
    audio.selectOutput(null);
    expect(audio.outputConfiguration?.deviceName).toBeNull(); expect(audio.pendingOutput).toEqual(pending);
    audio.pause(false);
    audio.selectOutput(named);
    audio.detachOutput(); expect(audio.outputState).toBe("detached");
    expect(audio.selectedOutput).toBe(named); expect(audio.outputConfiguration).toBeNull();
    expect(audio.sampleClock).toBe(clock); expect(music.sourcePosition).toBe(position);
    audio.selectOutput(named); expect(audio.outputState).toBe("playing");
    expect(() => audio.selectOutput("nonexistent-quake-audio-output")).toThrow();
    expect(audio.outputState).toBe("playing"); expect(audio.sampleClock).toBe(clock);
    expect(audio.mix(32)).toEqual(reference.mix(32));
    expect(audio.sampleClock).toBe(clock + 32);
    audio.stopAll(); expect(audio.pendingOutput.length).toBe(0);
});
