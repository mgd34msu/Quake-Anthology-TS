...
The mandatory seat fixture starts two local players with distinct inventories and layout strings, alongside a remote client. Each local player independently operates its wheel, menu, bindings, and gameplay inputs. Deliver seat-directed commands and a private beam, light, and temporary effect to each seat. Assert delivery to the intended owner, no disclosure to the other local or remote player, and no execution as the primary player. Deliver a shared world effect to all eligible views and assert exactly one effect instance per receiving client, with no duplicate simulation or presentation event. Repeat through CPU and OpenGL rendering, preserving each seat's HUD, prediction, listener, and command stream. This addresses concrete donor gaps: [Q1 splitscreen](../../quake-1-re-ts/src/client/splitscreen.ts), line 64, shares temporary entities, lights, and beams; [Q2 seat message decoding](../../quake-2-re-ts/src/client/cl_seats.ts), line 486, drops secondary `svc_stufftext`, `svc_layout`, and `svc_inventory` payloads.

The mandatory timing fixture fixes frame-entry order, equal-deadline tie order across behavior providers, command subdivision, due-think boundaries, and nested callback order. Its trace records RNG ownership and draws, timestamp rounding at each source operation, and when spawned or deleted actors become visible. Native cases follow their own source clocks, including classic Q2's 10 Hz game logic independently of wire encoding. Mixed cases exercise the explicit composition contract. Compare [Q1 `SV_RunThink` and physics](../../qsrc/quake/WinQuake/sv_phys.c), [Q2 `G_RunFrame`](../../qsrc/quake-2/game/g_main.c), and [Q3 `Pmove` and `PM_Weapon`](../../qsrc/quake-iii-arena/code/game/bg_pmove.c). Position agreement alone cannot pass prediction: under packet loss, late commands, and replay, assert weapon switching, ammunition, animation, impulses, and each predicted or authoritative effect exactly once. Required specimens include Q2's Q64 configuration and LMCTF wide-wire command adaptation, extending the bounded [classic wide-command test](../../quake-2-re-ts/test/protocol_classic_wide_move.test.ts).
...
The mandatory guest ownership fixture runs two independent sessions and creates a foreign actor in each. Guest instructions write that actor's bytes through partial and aliased accesses, including overlapping copies, and retain a pointer. Nested cross-family damage and touch callbacks delete the actor and reuse its slot. Assert every caller's observations before each nested return, mapped-field authority, private-byte preservation, source link-time bounds, stale-reference behavior, and saved interpreter context. A write in one session must never affect the other. No delayed copy-back or post-callback write masking may satisfy these assertions. Source obligations include [Q1 `OP_STOREP_*` and `OP_ADDRESS`](../../qsrc/quake/WinQuake/pr_exec.c), [Q2 edict layout and engine imports](../../qsrc/quake-2/game/game.h), [Q2 entity linking](../../qsrc/quake-2/server/sv_world.c), and [Q3 borrowed game data](../../qsrc/quake-iii-arena/code/server/sv_game.c).

After the nested callback completes, save, terminate the process, and restore in a fresh process. Verify actor identities, retained-pointer meaning, callbacks, imported resources, and interpreter state before continuing play. Resource handles and callback addresses must resolve to their restored owners rather than survive accidentally through the old process. Failure blocks dependent guest/composition work in the canonical graph; a successful loader cannot bypass this fixture.
