...
Actual Xatrix API3 and q2eaks API2023 checks cover firing, switching, ammunition, save continuation and retirement during a shot. These identify specific DLLs and declaration layouts, not every native mod. The q2eaks check is separate from retail rerelease qualification. Original native-primary selected arsenals, complete camera integration and broader artifact coverage remain in progress. These component item changes are newer than installed `434a4d3e`.
...
Actual Application checks cover Q2 firing, supply, cheats, save/load, death, respawn, travel and HUD visibility, plus Q1 Rogue/Q3 Team Arena held models and source Quad passes. This does not qualify other original server binaries, arbitrary Q1 model topology, mixed native primary movement/character combinations or portable foreign holdable/persistent equipment. Native Q2 primary selected arsenals remain unfinished.

Original Xatrix API3 and retail rerelease API2023 pickup callers now expose source-decided weapon and ammo supply. Original weapon ownership checks observe the selected arsenal, then restore original counters before recipient grants and map effects. Per-pool quantity evaluation runs original `Add_Ammo` with the selected count and capacity, restoring temporary values before returning and excluding unrelated power-armor effects. Original respawn and target callbacks remain in charge. Focused original-DLL checks cover ownership, refusal, capacity limits and restoration; the complete native-primary selected-arsenal join remains in progress.
