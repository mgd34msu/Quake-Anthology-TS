...
Nonstock QuakeWorld primaries can declare `combat` in their digest-bound `quakec-compatibility.json`, using the same source-call format as NetQuake. The declaration must name the original `T_Damage`, lower target/inflictor/attacker/amount in that order, and provide every additional original argument and required global. Admission validates original parameter types, immutable function references, armor inventory constants, and actual pain/death call sites before enabling the native combat observer. QuakeWorld keeps its own engine ABI and source damage, armor, momentum, and reaction code. Missing declarations remain rejected; pinned stock QuakeWorld needs no new metadata. Armor-region replacement and points-only armor grants still require their own qualified `armorStage` and `emptyArmor` declarations. This combat interface does not declare foreign-arsenal damage multipliers or weapon/client stages.
