...
The combat interface explicitly names `damageCall: "q3-g-damage-8-check-armor-3"`, retaining the original eight-argument damage and three-argument armor calls. Its required `state` maps the health stat, team persistent stat and namespaced team identities, actor flags, and constant or source-field mass. `damageFlags` maps the original radius, armor, knockback, protection and team-protection bits. The client pointer is a declared private field. Known flags translate at the shared boundary; unknown bits stay with their active original call and its transformed continuation. Independently initiated attacks use the common declared flag vocabulary. Different private call conventions require an additional source interface.
...
Nonstock QuakeWorld primaries can declare `combat` in their digest-bound `quakec-compatibility.json`, using the same source-call format as NetQuake. The declaration names the actual original damage function and maps `self` (target), `inflictor`, `attacker`, and `amount` through its typed arguments or globals. It supplies every additional original argument and required global; the function name and argument order may differ from stock. Repeated bindings for one semantic input must agree when original bytecode invokes the function. Admission validates original parameter types, immutable function references, armor inventory constants, and actual pain/death call sites before enabling the native combat observer. QuakeWorld keeps its own engine ABI and source damage, armor, momentum, and reaction code. Missing declarations remain rejected; pinned stock QuakeWorld needs no new metadata. Armor-region replacement and points-only armor grants still require their own qualified `armorStage` and `emptyArmor` declarations. Foreign-arsenal damage multipliers and weapon/client stages require their separate qualified declarations.


For selected foreign arsenals on an original QC primary, `combat.damageScale` declares the original attacker multiplier region: `{function, entry, exit, damage, statements}`. `damage` is the original float argument word; `statements` contains the exact instructions from entry through the join. The containing compatibility document binds the declaration to the program digest. The existing `combat.damage` call supplies the real attacker and current source time.

Admission proves that the region only reads attacker state and live source rules, computes a linear multiplier, and writes private scalar temporaries. It rejects target-dependent defenses, damage-dependent branches, additive damage, clamps, effects and escaping control flow. A single multiplication may use a live source factor; multiple scales require positive, nondecreasing powers of two so collapsing them does not change binary32 rounding. Division requires an exactly representable power-of-two reciprocal. Querying seeds damage with one and executes that original region, restoring its private frame and scratch words before returning. The same qualified region is skipped only when attack provenance identifies that source as the already-applied scaling owner. Other modifiers and target defenses continue normally.

Selected Q1, Q2 and Q3 weapons query the primary QC factor independently of the ordinary Quad flag. This retains additional original modifiers, such as the original Threewave Strength rune, including simultaneous Quad and Strength, without multiplying them again in `T_Damage`. The pinned classic Q1 and QuakeWorld paths retain their existing source rules. An unqualified mod multiplier rejects foreign-arsenal admission instead of substituting a fixed Quad value. The complete combat declaration remains part of the saved source identity.


The QC observer and incoming calls share the qualified damage-role layout. Transformed original calls change only the mapped target, inflictor, attacker and amount inputs. Extra original arguments, including vectors, keep their captured values rather than being rebuilt from declaration defaults. Changed input globals are restored when that invocation completes or fails, including nested source calls; untouched original globals retain source effects. Other damage metadata still requires replacement if a transform cannot be expressed by this interface. Original health, armor, movement stores and pain/death continuations remain source-owned. The linear scaling qualifier uses the same role map; its saved result still requires a qualified original float argument. Pinned Q1/QW declarations must retain their exact original role layout.
