...
`QTSAVE3` stores the independent layers and accepts old version-2 saves. Only a restored original source binding can normalize its exact old synthetic power-only placeholder; real depleted regular equipment remains intact. Unified frame/snapshot version 2 carries both layers. Foreign powered protection requires a declared original damage-stage adapter; the representation alone does not supply one.

Native `poweredProtection` declarations can bind a borrowed player’s powered slot independently of its regular armor and health owner. The declaration names its rule, original absorption entry, source flag dialect and scalar storage. Fuel storage must map to an existing declared canonical inventory item. Source `ClientCommand`/use callbacks own activation, including refusal without cells; the host does not synthesize successful activation or save a second fuel count. One owner may claim the slot. Replacing a primary owner requires its exact provider identity; conflicting components fail before original initialization.

Shared Q1/Q2/Q3 policies invoke that original power stage before regular armor. Qualified Xatrix and retail rerelease primary damage routines expose their original power stage as well, including internally authored hits. Their regular protection, health and reactions continue in original code. Other original damage owners, including currently unqualified QC/QVM stages, reject the combination at admission. A retired target cancels only its active native damage frame. Nested damage observes committed fuel before inventory observers run; transformed fuel counts are mirrored once. Inventory replacements that bypass the required canonical source store are rejected before post-store observers.

Component restoration and activation precede validation of composite protection and canonical fuel. Disable releases the component layer while preserving regular armor and inventory. Original modules can restore an inactive shield or different private auto-activation state; those source decisions are preserved. This integration does not establish full original movement/weapon loops or compatibility with undeclared module layouts.

Native owned armor declarations identify source scalar storage, ordered inventory or enum selectors, and fixed protection metadata. The original damage routine decides absorption and cell costs; shared combat observes its ordered armor and health writes. Source saves remain authoritative. Updating one armor tier preserves other held items; unrepresentable selections fail before changing storage. Full combat ownership remains limited to owned native actors. Borrowed powered protection uses the independent stage described above; borrowed regular armor and runtime-mutated protection metadata remain separate work.
