...
QuakeWorld component full weapon firing still needs the original `PF_aim` service. The current real-program check exercises `player_shot1` muzzle output, blood visibility and authored sound, not the complete original arsenal. NetQuake addon vector punch events also retain a separate simulation-state integration gap.
