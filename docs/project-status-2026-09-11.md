...
A separate selected dog/berserk roster change is also uncommitted. The implementer reports actual authored berserk club damage of 9 with knockback and saves, and a dog at slot 274 with bite damage of 12, leap damage of 13 and saves. Lead verification is pending. Six of 17 authored Q2 placements obstruct the dog; the map and roster remain unaccepted.

Complete independent swappability remains the main missing capability. Selected Q3 weapons and several source equipment mechanics work in bounded mixed configurations. Other foreign arsenals, the complete enemy roster, finer source cadence, map mission obligations, expansion supply mappings, VM-private overrides and mixed network state are unfinished. Full native Q3 saving and complete guest saving remain open. Rendering has inspected improvements, but lighting, sky and flash fidelity are not fully established; silent checks do not establish that audible chopping is fixed.

The installed `~/Projects/qfiles` executable remains the `827636f` bot build. It does not contain the latest engine corrections. Two Astra Medium workers continue under lead coordination with one Git writer. Menus remain paused. Implementation and useful live play precede optimization and broad qualification; focused checks continue before bounded commits.
