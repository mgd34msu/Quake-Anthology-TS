...
Current progress through pushed source `a667293` and installed `a43cd7c`: admitted Hipnotic/id1 NetQuake/QuakeWorld checkpoints survive disk save, close and fresh Application restore without replaying startup callbacks. Transactional world replacement preserves input/controller/audio ownership. Death presentation, Load/Restart UI and supported level autosaves are included. Newer source `a667293` joins Application native-Q3 and attached-bot saves, with actual fresh disk continuation on Q1/Q2/Q3, transactional bot-client restoration and native local autosaves after the first completed frame. Those newer source changes are not yet in the installed executable. Compiled `a667293` loaded q3dm1 but crashed on `addbot sarge` in renderer transform handling before save/load; the exercised graphical bot route is blocked and compiled recovery remains unqualified. QVM/guest persistence, source-native save-file compatibility, complete campaign recovery and broader bot modes/maps remain open. The latest compiled checks cover startup, movement and 30-second firing on two Q2 mixed recipes; compiled save/load recovery was not exercised. See [execution status](execution-status.md#installed-executable-and-recent-fixes) for evidence and the executable hash. The historical findings below retain their original cutoff.
