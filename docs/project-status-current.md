...
Current accepted source through `8ec0b01` includes admitted QC checkpoints, native-Q3 and attached-bot restoration, and local two-seat QVM disk restoration with transactional world replacement. Startup restore now constructs from the saved state without overwriting its autosave or replaying source startup callbacks. Newer source also adds shared debug shapes, live `cl_timeNudge`, Q2 effects sounds and speaker/loop-gain corrections. Installed `7b8a193` passes fresh compiled startup scrolling/value/binding checks, including clear persistence in a second process. The four public load/input/save/exit flows remain historical `8ec0b01` evidence, not rerun gameplay qualification for `7b8a193`. Guest bots/travel, live-network saves, native DLL/SO execution, required source save formats, full campaigns and broader compatibility remain open. Q2 black triangular patches remain a visual finding under investigation; the original quiet Q3 rocket report remains unresolved. See [execution status](execution-status.md#installed-executable-and-recent-fixes) for source and executable evidence. The historical findings below retain their original cutoff.
