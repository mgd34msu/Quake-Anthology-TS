...
Current progress through pushed `8084b42` and installed `a6cb2e0`: admitted Hipnotic/id1 NetQuake/QuakeWorld checkpoints survive disk save, close and fresh Application restore without startup callback replay. Newer source also joins Application native-Q3 and attached-bot saves, with fresh disk continuation on Q1/Q2/Q3, transactional client restoration and native local autosaves after the first completed frame. Dedicated offline LRCTF QVM saves now restore fresh with exact VM/server continuation. Live-network saves, local graphical QVM saves, guest bots/travel, source-native formats, full campaign recovery and broader bot modes/maps remain open. The installed executable passes bounded native Q3 bot save/load, post-load input, a second save and normal exit, plus 30-second firing on two Q2 mixed recipes. Exact continuation and QVM recovery retain separate source-test evidence. Newer pushed source adds bounded two-seat local QVM play, tested for 1,150 ms of simulation with 650 ms of input, and restores missing Q2 rocket/grenade effects sounds. Both changes remain uninstalled; local QVM saves/bots/travel remain guarded and the original quiet Q3 explosion report remains unresolved. See [execution status](execution-status.md#installed-executable-and-recent-fixes) for evidence and the executable hash. The historical findings below retain their original cutoff.
