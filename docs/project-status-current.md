...
Usability is the highest product priority across client menus and console commands. Completion requires sensible defaults for selected content, visible values, direct mouse/keyboard editing, familiar commands, clear feedback, persistent settings and an actual menu/runtime check. Committed source now includes native game/edition presets, a shared binding table and console binding/configuration corrections; these updates are not installed. Startup binding access, defaults and archive work remain in progress. See the [current priorities](implementation-priorities.md#highest-product-priority-usable-controls-and-console).

Current accepted source through `d07708e` includes admitted QC checkpoints, native-Q3 and attached-bot restoration, and local two-seat QVM disk restoration with transactional world replacement. Startup restore now constructs from the saved state without overwriting its autosave or replaying source startup callbacks. Newer source also adds shared debug shapes, live `cl_timeNudge`, Q2 effects sounds and speaker/loop-gain corrections. Installed `a6cb2e0` retains its bounded native Q3 bot save/load and two Q2 mixed-recipe firing proof; the later source changes are not installed yet. Guest bots/travel, live-network saves, native DLL/SO execution, required source save formats, full campaigns and broader compatibility remain open. The original quiet Q3 rocket report remains unresolved. See [execution status](execution-status.md#installed-executable-and-recent-fixes) for source and executable evidence. The historical findings below retain their original cutoff.
