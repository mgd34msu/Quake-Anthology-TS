...
The lead independently verified representative offhand grapple attachment, active-tether save continuation, LMCTF pause/disconnect, and native Q3 damage against a foreign rerelease hook. Source slot adapters share native animation operations, but application weapon-slot selection, outgoing view projection and slot-state saves are still being integrated. Native Q1 sky removal and an actual moving Q3 brush anchor remain unproven. GUI/keybinding wiring is absent. Next are shared slot application, selected-monster admission with authored path/mission obligations, and general native Q3 views of foreign source actors.
