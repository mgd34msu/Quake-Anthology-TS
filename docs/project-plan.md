...
The next equipment dependencies are offhand grapple runtime integration with native disabled controls, then the source weapon-slot handoff coordinator/API, with independent work proceeding in parallel. Both are active work, not completed runtime claims. GUI/keybinding wiring is absent. Selected-monster admission, authored path/mission obligations and presentation joins remain pending.
