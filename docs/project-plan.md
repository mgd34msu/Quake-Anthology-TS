...
This plan builds one Bun engine for Quake 1, Quake 2 and Quake 3, including their classic and rerelease editions, all expansions, all game content and all functionality. Completion includes full campaigns, multiplayer, bots, local splitscreen, saves, demos, media, input, menus, server operation and mods. Maps, movement, characters, weapons, entities and rules can mix across games, including different selections for actors in one session. Implementation is authorized. The preceding implementation checkpoint `ec2686b` joined native server protocols, Q2 rerelease lighting/contacts and LMCTF pause/voting. The latest source checkpoint is `cdc2cc5a727f6e631668c7243c2e903977c160bc`, from verified snapshot `protocol-carry-CcxluG`, tree `e1b601d2f3a3a9c3e4d0b0b343c79906322a63e0`. The engine is now in official-content completion and full-application integration, feeding W50 and W66. W03 remains the only formally accepted package, for its early tooling scope. The [execution ledger](execution-status.md) separates current evidence from historical dispatches.

The lead verified compiled binary `950ff3ca812d28bc6935ad018cd6546dfcb5f71e62ccfeae37cf2362e8117a14` on CPU with `q1-classic-id1/e1m2`, Q1 movement and character, and on GL with `q2-classic-lmctf/lmctf09`, Q2 movement and character and two seats. Both used `listen 0` and exited successfully. The snapshot adds Q1 sounds/effects and frozen precaches, Q3 pure/archive downloads, LMCTF match travel/settings, QC spatial builtins over shared raw bodies, and actual Q1 key paths. Exact per-map QC precache ordering is not established. These bounded results do not accept full packages, campaigns, source parity or every configuration.

Player bots are being consolidated into one Q3-derived core with useful rerelease capabilities, issuing normal actions under engine physics. Production admission remains Q3-only until that refactor is proven. An alternate rerelease player-bot brain is not the target architecture. Native wire support covers family-compatible configurations; mixed transport and full QC application lifecycle/monster builtins remain unfinished. A native rerelease retail run covered 502 edicts through initialization, spawning, client admission and active frames; full-map JSON saving exceeded the 20-million-instruction budget. LMCTF has no soundtrack of its own, as confirmed by the user. Native full gameplay/save coverage, complete CPU/GL content coverage, multiplayer interoperability and all declared configurations remain open. Follow the user's code-first sequence: finish imports and live gameplay, optimize, then complete broad hardening and release evidence at W67–W71.
