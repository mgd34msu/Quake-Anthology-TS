...
This plan builds one Bun engine for Quake 1, Quake 2 and Quake 3, including their classic and rerelease editions, all expansions, all game content and all functionality. Completion includes full campaigns, multiplayer, bots, local splitscreen, saves, demos, media, input, menus, server operation and mods. Maps, movement, characters, weapons, entities and rules can mix across games, including different selections for actors in one session. Implementation is authorized. The latest committed implementation checkpoint is `f87e5b86256e5b8ec0832f1ecc9e1b9691a4e5f6`, following the shared application, combat/save, and interpreted native guest checkpoints. The engine is now in official-content completion and full-application integration, feeding W50 and W66. W03 remains the only formally accepted package, for its early tooling scope. The [execution ledger](execution-status.md) separates current evidence from historical dispatches.

The lead verified compiled binary `2ccfc8c20ae7f2277cff1c815bb04dc8cfb0335a14db992a487efa651cbaf6ac` on CPU with Machine `dm1`, Q2 movement and Q2 male, and on GL with LMCTF `lmctf09`, Q3 movement, a Q1 player and two seats. Both exited successfully. The checkpoint adds Machine navigation/path entities and rocket ogre behavior, rerelease medic revival metadata, and LMCTF administration, plasma and signed ammunition. The standalone arena bot fixture now passes after correcting its source skill to 3. These bounded results do not accept full packages, campaigns, source parity or every configuration.

Lighting, player contacts and skip voting have additional completed live-tree work awaiting a checkpoint. Current joins include NetQuake protocol 15 and Q3 protocol 68 server application paths and shared LMCTF pause. LMCTF has no soundtrack of its own, as confirmed by the user; its music warning is not a missing-content acceptance gap. A native rerelease retail run covered 502 edicts through initialization, spawning, client admission and active frames; full-map JSON saving exceeded the 20-million-instruction budget. Native full gameplay/save coverage, complete CPU/GL content coverage, multiplayer interoperability and all declared configurations remain open. Follow the user's code-first sequence: finish imports and live gameplay, optimize, then complete broad hardening and release evidence at W67–W71.
