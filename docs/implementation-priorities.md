...
Installed `a6cb2e0` includes death presentation, recovery UI and supported level autosaves, with actual CPU/GL death/load/restored source captures. Admitted Hipnotic/id1 NetQuake/QuakeWorld checkpoints now survive disk save, original Application close and fresh restore without startup callback replay. Application world replacement preserves held inputs, controller settings and audio resources. Newer pushed source `a667293` joins Application native-Q3 and complete attached-bot saves, with actual fresh disk continuation on Q1/Q2/Q3, transactional bot-client restoration and native local autosaves after the first completed frame. Current pushed `a6cb2e0` also joins dedicated offline LRCTF QVM disk restore, with exact VM/server continuation after fresh Application restore and no startup export replay. Installed `a6cb2e0` also passes bounded native Q3 bot save/load, post-load input, a second save and normal exit. Exact continuation and QVM recovery retain separate source-test evidence. See [execution status](execution-status.md#installed-executable-and-recent-fixes) for evidence. Live-network saves, local graphical QVM, guest bots/travel, source-native save formats and full campaign recovery remain open. Compiled a6cb checks also exercised 30-second firing on two Q2 mixed recipes. Local graphical QVM integration and the Q2 effects sound correction remain uninstalled work. This priority remains open.
