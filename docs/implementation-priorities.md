...
Installed `a43cd7c` includes death presentation, recovery UI and supported level autosaves, with actual CPU/GL death/load/restored source captures. Admitted Hipnotic/id1 NetQuake/QuakeWorld checkpoints now survive disk save, original Application close and fresh restore without startup callback replay. Application world replacement preserves held inputs, controller settings and audio resources. Newer pushed source `a667293` joins Application native-Q3 and complete attached-bot saves, with actual fresh disk continuation on Q1/Q2/Q3, transactional bot-client restoration and native local autosaves after the first completed frame. These newer source changes are not in installed `a43cd7c`. Compiled `a667293` loaded q3dm1 but crashed in renderer transform handling on `addbot sarge` before save/load, blocking the exercised graphical bot route and leaving compiled recovery unqualified. See the current [execution status](execution-status.md#installed-executable-and-recent-fixes) for the failed candidate receipt. QVM/guest persistence, source-native save formats and full campaign recovery remain open. Compiled a43 checks exercised startup, movement and 30-second firing on two Q2 mixed recipes, but did not exercise save/load recovery. This priority remains open.
