...
Installed `a43cd7c` includes death presentation, recovery UI and supported level autosaves, with actual CPU/GL death/load/restored source captures. Admitted Hipnotic/id1 NetQuake/QuakeWorld checkpoints now survive disk save, original Application close and fresh restore without startup callback replay. Application world replacement preserves held inputs, controller settings and audio resources. Newer pushed source `a667293` joins Application native-Q3 and complete attached-bot saves, with actual fresh disk continuation on Q1/Q2/Q3, transactional bot-client restoration and native local autosaves after the first completed frame. Current pushed `a6cb2e0` also joins dedicated offline LRCTF QVM disk restore, with exact VM/server continuation after fresh Application restore and no startup export replay. These newer changes are not in installed `a43cd7c`. The earlier graphical native-bot crash is diagnosed and source-fixed; compiled recovery remains pending. See [execution status](execution-status.md#installed-executable-and-recent-fixes) for evidence. Live-network saves, local graphical QVM, guest bots/travel, source-native save formats and full campaign recovery remain open. Compiled a43 checks exercised startup, movement and 30-second firing on two Q2 mixed recipes, but did not exercise save/load recovery. This priority remains open.
