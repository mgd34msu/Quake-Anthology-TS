...
Accepted `5258ee0` binds all 18 UI LAN browser calls through shared discovery and cache ownership. IPv4 and JSON cache scope and native admission gates remain. A bounded artifact-only gate bypass reached one active frame with real mounted LRCTF UI/cgame modules after UI71 completion, against a baseq3 server advertising fs_game=lrctf with sv_pure=0. That artifact probe left production admission unchanged; the later source acceptance below admits parsed unknown client bytecode. LRCTF server rules, pure mods, full gameplay, missionpack, and comprehensive unknown-module compatibility remain open. Accepted `1aa524d` reuses browser reads with root 19 tests/254 assertions.
