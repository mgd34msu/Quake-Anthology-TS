...
Accepted source includes five native game/edition presets with campaigns, the shared binding table, canonical mouse names, readable binding queries and shared console/configuration execution. Startup binding access, selected-content defaults, numeric slider values and archived settings have actual source/runtime checks; existing profile bindings are preserved. Team Arena remains pending. Newer accepted source `63fe5d1` adds optional rerelease map-list shuffle and primary-world native Q1/Q2 competitive-level adaptation, with actual rerelease travel evidence and source/pure checks for Q1; it is not in installed `d1b437e`. Original source configuration startup execution has bounded acceptance; dynamic commands and CLI `stuffcmds` remain open. The four public gameplay flows at `8ec0b01` and startup scrolling/value/binding plus second-process clear checks at `7b8a193` are historical evidence. See [execution status](execution-status.md#installed-executable-and-recent-fixes) for the current installed identity and acceptance boundary.
