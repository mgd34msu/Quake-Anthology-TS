...
The installed executable is `72794d58`. It includes the Q1 PUSH travel correction, managed native integer blocks and presentation between completed catch-up ticks. The latest matched complete original base1 comparison measured 76.34 to 74.61 ms per native call and 84.31 to 82.85 ms per application step, retaining all 495 source entity records. This remains above the original 25 ms interval; T10 is open. See [execution status](../execution-status.md#installed-executable-and-recent-fixes) for delivery evidence and [tick execution](../tick-execution.md) for the cycle breakdown. These bounded measurements do not establish general rendered FPS.
