...
The current executable is `71f0e4`, including qualified GL array-layout reuse and direct native scalar stores. Its direct QuakeWorld-movement gameplay/save/Quit check passed; this was not a performance measurement. See [execution status](../execution-status.md#installed-executable-and-recent-fixes) for delivery checks and runtime evidence. Older installed/pending labels below describe their recorded checkpoints. Current whole-game FPS is not established by those component experiments.
