...
The installed executable is `726d72fb`. It includes retained native call plans and weapon-model lists, the Q1 PUSH travel correction, managed native integer blocks and presentation between completed catch-up ticks. The latest cache comparison measured about 1% less native-call time in both pairs; whole-application timings overlap. Native calls still take roughly 75 ms against the original 25 ms interval; T10 is open. See [execution status](../execution-status.md#installed-executable-and-recent-fixes) for delivery evidence and [tick execution](../tick-execution.md) for the cycle breakdown. These bounded measurements do not establish general rendered FPS.
