...
Classic and rerelease cvar adapters now retain their stable field addresses when each guest record is allocated. Each refresh avoids recreating six field pointers and, for rerelease, looking up those field offsets again. Registry values, native writes and source list order still refresh as before. Focused cvar/service checks and the original rerelease API attachment check pass, with scoped strict TypeScript clean. This removes repeated metadata work; no whole-frame timing gain is claimed.
