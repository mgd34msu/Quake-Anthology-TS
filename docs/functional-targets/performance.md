...
The installed executable is `c80aed27`. It includes native code-page write tracking, direct integer register and ABI conversion, safe single-instruction managed blocks, retained call plans and weapon-model lists, the Q1 PUSH travel correction, and presentation between completed catch-up ticks. The latest matched comparison reduced native calls from 68.49 to 66.78 ms and application steps from 77.44 to 75.28 ms, with identical instruction counts and final native entity bytes. Calls still exceed the original 25 ms interval; T10 is open. See [execution status](../execution-status.md#installed-executable-and-recent-fixes) for delivery evidence and [tick execution](../tick-execution.md) for the cycle breakdown. These bounded measurements do not establish general rendered FPS.
...
Build derived data once and update it when its inputs change wherever that removes useful work. Cache stable results at the owner that can observe every relevant change, including guest writes, replacement, map transitions and save restoration. Use the ordinary computation when invalidation would be unreliable or cost more than rebuilding. Events, arbitrary game callbacks and shared mutable simulation steps still execute in source order.
...
SSE arithmetic and x87 instructions now also retain their decoded fields. Register and memory operands, rounding controls, exceptions and stored results remain live; memory operations end their prepared block so stores cannot bypass code or callback checks. Matched base1 pairs measured native medians of 67.07 → 65.55 ms and 67.16 → 66.54 ms. Application medians were mixed: 74.96 → 75.45 ms and 76.45 → 75.66 ms. All instruction counts and final entity bytes match. The reverse pair counted 200 top-level and 50,731 nested ABI calls in each variant. Focused CPU/memory/ABI/floating checks passed 114 tests with 2,042 assertions; three-root strict/policy passed. Evidence is local in `.artifacts/resume-20260925/numeric-plans/`. Terminal stores in managed blocks and narrow register stores were also tried and removed because their comparisons did not establish a gain.
...
Repeated entity reconciliation and cvar synchronization remain candidates. Their invalidation must cover original DLL writes and callbacks; the caches above do not skip those operations.
...
The current executable includes the MD3/MD2, GL layout, and scalar-store changes. [Execution status](../execution-status.md#installed-executable-and-recent-fixes) records its identity and runtime qualification. The earlier `065259c` [exact-binary GL follow-up](../../.artifacts/resume-20260918/compiled-mixed-065259c/RESULT.md) passed a bounded mixed scene and normal Quit with unchanged guards. Neither that check nor delivery of the later changes is a performance comparison. The component gains above remain the measured performance evidence.
...
The [actual native component workflow](../../.artifacts/resume-20260918/scalar-store-native/receipt.json) also passed public external-profile installation, Q2Eaks code controlling a Q3 rocket, save/load, immediate restored state, and matching 500 ms continuation. All 2,582 inputs remained unchanged and processes were reaped. Its 56.801-second elapsed time is correctness evidence from one run, not a native performance comparison. The current executable includes this qualified source change; this source workflow is not a gameplay check of the new executable.
...
The composed two-file change passed strict TypeScript and scoped source policy checks with 288 loaded files. All 2,578 guarded inputs were unchanged; the existing collision suite passed 10 tests with 129 assertions, including installed `start.bsp` and `e1m1` geometry. This qualified change is included in the current executable; exact identity and delivery limits are tracked in [execution status](../execution-status.md#installed-executable-and-recent-fixes).

## Verified archive reuse during preparation

One disposable preparation scope now shares verified archive descriptors among inherited products during behavior discovery and rerelease start-map lookup. Hashing and archive parsing use the same retained file descriptor; entry reads still check for in-place changes. Product order, identity, restrictions, and unavailable-choice results remain separate. The scope closes borrowed resources after preparation, including failed and pending acquisitions.

One completed A/B/B/A comparison over 30 installed products measured discovery at 12.200 / 5.732 / 5.611 / 12.130 seconds, a 53.38% aggregate reduction. Complete choice arrays, including declarations and errors, matched. Each pass referenced 193 archives with 63 distinct identities; repeated verification volume fell from 22.36 GB to 5.91 GB. Sampled peak archive descriptors increased from 29 to 63, with zero left open after each pass. All 63 archive digests and 2,493 source/driver guards matched. This measures behavior discovery, not total startup time or gameplay FPS. The earlier serialization-only fixture failure and corrected attempt remain recorded. [Discovery evidence](../../.artifacts/resume-20260918/menu-archive-scope/timing2/result.json).

## Conservative split classification

Immutable cell bounds now certify splits that are entirely in front, behind, or inside the original tolerance band. The two corner expressions retain the original arithmetic order. Nonfinite or inconclusive bounds fall through to the unchanged classification and clipping code; no intersection, cap order, epsilon, or geometry changes. Weak keys release bounds metadata when their cells die.

All 15,000 recorded TraceResults matched exactly, including signed zero, as did 1,620 synthetic traces. The single A/B/B/A trial reduced aggregate replay time by 36.86% for fresh owners, 34.56% for warm owners, and 40.57% for unique-envelope misses. These are pure collision measurements over the retained short capture, not whole-game FPS. [Complete results and scope](../../.artifacts/resume-20260918/split-bounds-e689603/RESULT.md).

Both changes passed composed strict TypeScript and scoped source-policy qualification. The final test-only namespace correction was rechecked; all 2,578 guarded inputs remained unchanged. The joined split and preparation tests passed four focused cases. This qualified source is newer than the installed executable recorded in [execution status](../execution-status.md#installed-executable-and-recent-fixes).
