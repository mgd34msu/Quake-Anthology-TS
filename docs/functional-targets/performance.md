...
The installed executable is `6357117f`, including native instruction, memory, SIMD and exact integer improvements. Its archived source identity, build/help and compiled original native GL check passed. Newer accepted source reduces the complete original base1 sample to a 73.59 ms median native call and 81.02 ms application step, with all 495 source entities retained. This remains above the original 25 ms interval; T10 is open. See [execution status](../execution-status.md#installed-executable-and-recent-fixes) for delivery identity and bounded measurements. These measurements do not establish general rendered FPS.
