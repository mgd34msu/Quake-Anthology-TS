...
| T07 | Network connections and prediction | Active | Mixed-game UDP sessions now negotiate content, predict movement, retain local seats and survive travel/restart. DOSBox IPX is selectable for NQ/Q2/Q3. Native AF_IPX backend and KEX 2023 lobby transport remain implementation work; broader native-peer interoperability coverage is deferred. |
...
The accepted IPX slice makes DOSBox IPX selectable in NQ, Q2 and Q3 and honors the physical packet ceiling. Native `AF_IPX` remains explicitly unavailable. Final strict/policy checks passed with 2,425 unchanged inputs; central checks passed 23 tests/188 assertions and Q3 checks passed 23 tests/169 assertions. No native DOSBox peer run or new executable delivery is claimed. The subsequently integrated mixed-game transport is described below. Native IPX and KEX transport keep T07 active: **7/23 complete, 16 open**. [Qualification receipt](../../.artifacts/resume-20260917/t07-ipx-check2/receipt.json).
...
T07 stays open for the native IPX socket backend and KEX 2023's lower lobby/session protocol. Public KEX documentation supplies game-level connection messages but not the required outer transport. Native-peer matrices remain unrun coverage; additional testing infrastructure is deferred while implementation proceeds. Native guest execution and unified recording retain their T10 and T20 boundaries. The installed executable is still the separately identified build above until delivery completes.
