...
**22/23 functional targets complete in accepted source; T19 remains open.** T10 now includes the external native declaration path and saved profile identity. Those latest changes are qualified source awaiting executable delivery; installed `07d54ae` retains the preceding built-in profile. Source acceptance covers implementations and their public consumers. Full campaigns, native peers, physical devices, and whole-game performance retain the explicit limits below.

Installed executable is `07d54ae859ee757d6e446fa8f29a7eb3ec20ed7b` and includes the preceding accepted source changes. Exact installed identity is recorded in [execution status](../execution-status.md).
...
| T10 | Gamecode and mod execution | Complete in accepted source | Original QuakeC, classic/rerelease native modules, QVM game/client/UI adapters, foreign maps, and declared QC/native/QVM projectile components have consumers. Real mod firing/save workflows passed. External native declarations, artifact/layout admission and full saved profile identity now pass public installation and actual native saved continuation; built-in profiles use the same binder. Explicit legacy ABI limits remain. |
...
| T12 | Combat, rosters, pickups and equipment | Complete in accepted source | Expansion monsters, supply and source pickup policy, powerup/power-armor HUD, boss health and per-seat item visibility have consumers. Four mixed-expansion flows and the routed pickup-policy correction have focused evidence; exhaustive roster/weapon combinations remain unclaimed. |
...
| T19 | Progression and player services | Active | Durable local records, progress browsing, arena/Team Arena consumers and Q3 ranking producers/account menus are joined. Shared lobby lifecycle and authoritative native match reports are joined. Original GRank transport/service compatibility is unavailable and is not replaced by invented wire behavior. |
...
Active extension work: move native trajectory selection from a built-in digest/factory registry to validated package declarations. Persist the full normalized declaration so unchanged DLL bytes cannot hide changed layout/callback data during restore. Existing whole-module execution and the built-in native component remain available.
...
No physical scale conversion was justified by the inspected source dimensions. Native pairs keep factor 1, and selected body/view dimensions remain source-owned. The original closed engine's corpse behavior and exhaustive native-peer/body combinations are not claimed as verified. Those coverage limits do not represent missing movement handlers. T12 and T10 are accepted. `07d54ae` contains their preceding component and pickup changes; the latest generic native declaration extension is qualified source awaiting the next executable.
