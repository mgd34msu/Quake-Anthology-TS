...
The original rerelease tick interval here is 25 ms, or 40 ticks per simulated second. Classic native Q2 uses 100 ms. The latest cache comparison measured 74.57–74.89 ms inside the native RunFrame call and 83.83–85.05 ms for the application step, with 495 entity records retained. RunFrame time includes engine callbacks invoked by the DLL. These are separate medians from two bounded runs, not an additive phase profile or rendered FPS result.
