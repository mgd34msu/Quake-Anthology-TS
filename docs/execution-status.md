...
Checkpoint `e20b6d6` adds ordinary Q2 rerelease roster admission and shared monster muzzle audio, including extended rerelease flash IDs. The common effects consumer now handles berserk slam and plain explosion; classic monsters emit the actor-bound typed muzzle event. Actual resource closure checks and independent roster, native source, dummy-audio and CPU effects tests passed 36 tests with 615 assertions. This source checkpoint is included in the installed `43924e3` executable.
...
Checkpoints `e973079` and `c12153d` fix common selected-weapon presentation and share Q1 client visibility between native and QC hosts, including saved cache state and source field projection. Root independently passed runtime and render/HUD checks (33 tests/1,144 assertions, with one pre-existing SDL skip) and inspected eight CPU/GL e1m1/q3dm1 captures. The final frozen source passed strict and policy after removal of an unused helper and duplicate import; root reviewed those exact cleanups. The controlled Q3 prepared-scene capture predates shotgun animation, and false HUD inventory and overlap remain separate limitations. Visibility sharing does not complete guest frame physics or application guest lifecycle. These four commits are pushed but are not in the installed `43924e3` executable.

Checkpoints `38042e2` and `703cc0a` add QC movetogoal execution, correct selected-source prediction angles and project canonical weapon/ammo status and icon resource metadata for native, selected and remote arsenals. Root independently passed 41 tests with 1,753 assertions across source, prediction, catalog/assets and native UDP checks. Five actual retail cases confirmed source provider/content and ammo transitions: Q1 super-shotgun transitions from one shell to zero; Q2 BFG is blocked at 49 cells, ready at 50 and consumes 50. The QC startup probe completed 23 startup callbacks and 12 actual movetogoal calls; it does not establish complete guest frame physics. The display consumer remains separate work: canonical HUD data and resources do not yet prove that the displayed HUD is correct. The frozen source passed strict TypeScript followed by policy. Both commits are pushed but absent from the installed `43924e3` executable.

## Installed build

The Linux x64 executable at `/home/buzzkill/Projects/qfiles/quake-typescript` is built from checkpoint `43924e30c290fa1d657ff9a7b6c62617f52352af`, replacing the earlier `ebce976` binary. Running it without arguments opens the main menu before creating a world, player or game audio. Explicit CLI game/preset launches remain supported. Bun 1.3.14 produced the 119,130,240-byte executable from immutable source; strict typing, policy and input-stability gates passed. Installation was atomic with mode `0755`.

- Source SHA-256: `8436ba283e75520d527c028c649f59bf8fc5ee1f0362c58e19c1fcb0878dab9c`.
- Executable SHA-256: `a957c255bea5a52363452978af4a3fd2f263ba96c66482f553470d00c99afb66`.
- Adjacent provenance: `quake-typescript.build.json`; previous binary and sidecar: `quake-typescript.previous-20260912T164555816Z` and its `.build.json`.
- Root independently verified the installed SHA-256, 119,130,240-byte size, mode 0755 and sidecar. Installed help exited 0; an eight-second offscreen/dummy no-argument menu run stayed open until SIGTERM, exited 0 and produced no stdout or stderr.
...
| W18/W19/W59 presentation | Checkpoint `423101a` fixes Q2 intensity, MD2 culling, Sarge child skins, audio queuing/frame cost and selected-character jump audio. | The installed executable is updated through `43924e3`; see the [build record](#installed-build). Shared startup/pause menus have inspected CPU/GL captures; complete presentation coverage remains open. |
