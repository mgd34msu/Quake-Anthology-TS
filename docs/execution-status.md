...
Checkpoints `8f91da4` and `6d7c253` add shared pickup-grant previews and remove unsupported Q1 crouching from bot navigation. Independent checks passed seven supply tests with 147 assertions and three posture tests with 94 assertions. Checkpoint `eb0d55f` runs the same Q3-derived bot core on Q1 standard deathmatch with selected base-Q2 weapon knowledge, preserving existing Q2/Q3 behavior. Frozen checks passed the Q1/Q2 application tests (2 tests/42 assertions), native Q3 application tests (2/15) and catalog tests (3/22). Actual probes cover respawn resetting rail decision 10 to starter shotgun 2, teleport retaining weapon preference, actor lifetime reuse, and unsupported bot arsenals leaving the game loaded. Q1 dm4→dm5 travel retains the exact admitted composition, module artifacts and asset precedence while resolving the new map. Native Q1 and foreign Q3 bot arsenal support, pickup goals and remaining source-ID tactical comparisons are incomplete. These source commits are newer than the installed `7cbab8d` executable.

## Installed build

The user-requested Linux x64 executable update installs commit `7cbab8dfa62495e48bbe33167234728728ac4a92` at `/home/buzzkill/Projects/qfiles/quake-typescript`, replacing the older `827636f` build. It includes the lighting, selected Q2 arsenal, barrel and shared target fixes through that commit. The unchanged `tools/build.ts` ran against the verified immutable source, with TypeScript, policy and input-stability checks passing. Bun 1.3.14 produced the 116,824,192-byte executable, installed atomically with mode `0755`.

- Source SHA-256: `dea90f612874114d613088e8eaf2f677a1dc13e23ca56a43d4733cb40b8cec67`.
- Executable SHA-256: `35315675d9e2f286285434ae115dbeb51e57fc81c49a3128f01b9ac877ea2bc8`.
- Adjacent provenance: `quake-typescript.build.json`; previous binary: `quake-typescript.previous-20260912T032708Z`.
- Installed `--help` exited 0. The installed executable also exited 0 for `SDL_AUDIODRIVER=dummy SDL_VIDEODRIVER=offscreen ~/Projects/qfiles/quake-typescript --preset q2-q1-q3 --content-root ~/Projects/qfiles --dedicated --frames 1`, loading actual `maps/base1.bsp` with Q1 movement and Sarge. No player client or audible session was launched.

The current player command is `~/Projects/qfiles/quake-typescript --preset q2-q1-q3 --renderer gl --gamma 1.3`. This preset uses Q2 weapons; the CLI has no `--weapons` or `--arsenal` option. Gamma defaults to 1 and accepts 0.5 through 3. The complete composition menu, audio fidelity, player-session qualification and release coverage remain unfinished. The installed help and dedicated smoke do not replace those checks.
...
The user's architecture rule applies to every subsystem: coalesce the strongest implementation and add every useful feature from every game to that common system. One engine, world and executable must expose those capabilities across content. Source-specific policies preserve required behavior. The latest user direction requires full independent swappability and prioritizes engine interoperability; menu work is paused.
...
| W18/W19/W59 presentation | Checkpoint `423101a` fixes Q2 intensity, MD2 culling, Sarge child skins, audio queuing/frame cost and selected-character jump audio. | The installed executable is updated through `7cbab8d`; see the [build record](#installed-build). Complete presentation coverage remains open; menu work is paused for core engine interoperability. |
...
The user confirms LMCTF has no soundtrack of its own. The music check verified `lmctf09` requests track 1 and decoded inherited Q2 track 02 without changing playback or files; its warning does not establish missing LMCTF content. The immediate work is selected Q1 arsenal/supply integration and remaining monster placement, cadence and mission coverage, then combined engine interoperability evidence. Menus W60/W61 remain paused. The graph still requires official gameplay → W50/W51 → W66 and W66 plus native execution → W40, with menus required before full W66 acceptance. W67 also requires independent evidence W63–W65, content completeness W07 and performance W68. Broad hardening follows imports, live human play and optimization under the user's code-first sequence. No full package is accepted by this refresh.
