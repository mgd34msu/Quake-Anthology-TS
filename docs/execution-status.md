...
Checkpoint `e20b6d6` adds ordinary Q2 rerelease roster admission and shared monster muzzle audio, including extended rerelease flash IDs. The common effects consumer now handles berserk slam and plain explosion; classic monsters emit the actor-bound typed muzzle event. Actual resource closure checks and independent roster, native source, dummy-audio and CPU effects tests passed 36 tests with 615 assertions. This source checkpoint is included in the installed `36c450b` executable.
...
The Linux x64 executable at `/home/buzzkill/Projects/qfiles/quake-typescript` is built from `36c450bd1fdbf00ad3f1332c56ca5e8707029c01`, replacing `43924e3`. Without arguments it opens the main menu before creating a world, player or game audio. Bun 1.3.14 produced the 119,224,448-byte executable from immutable source; strict typing, policy and input-stability gates passed. Installation was atomic with mode `0755`.

- Source tree: `2572d79333a41aa66898370134c176bfc0d91605`.
- Source SHA-256: `17d64b47a4e7080952b6b792060389ac9d8a11e7b921db11cbd93d97b4f85482`.
- Executable SHA-256: `deb0d8a226057d2242c15a289e35c3a5508f3480cf872a5aa98cd03adc829e69`.
- Provenance: adjacent `quake-typescript.build.json`; previous executable, sidecar and instructions use backup prefix `quake-typescript.previous-20260912T190843739Z`.
- Root independently verified the installed executable SHA-256, 119,224,448-byte size, mode 0755 and sidecar commit/tree/source hashes. Installed help exited 0; an eight-second offscreen/dummy no-argument run stayed alive until SIGTERM, exited 0 and produced no stdout or stderr. The process closed cleanly.

Development saves from older builds are incompatible: Q1 foundation save version 5 rejects version 4, and the previous installed `43924e3` build is older still.

Start with `~/Projects/qfiles/quake-typescript`. The mouse-driven main menu offers Single Player, Multiplayer, Load Game, Options and Quit. Configuration resolves independent supported selections into the exact recipe on Play; failures return to the retained draft. Load Game lists actual saves with metadata and unavailable-save diagnostics. Display Apply changes the actual renderer; Sound/Controls reuse in-game bindings. Independent frozen probes passed actual Play/save/load, preferences, display Apply and runtime-failure return; CPU/GL menu and pause captures were inspected. These results do not establish complete campaigns, arbitrary pickup replacement, Q3 full saves, server/lobby/bot administration, audio fidelity or full rendering parity. Installed Quake III or rerelease font assets are currently required for proportional menus. No audible desktop session was launched.
...
| W18/W19/W59 presentation | Checkpoint `423101a` fixes Q2 intensity, MD2 culling, Sarge child skins, audio queuing/frame cost and selected-character jump audio. | The installed executable is updated through `36c450b`; see the [build record](#installed-build). Shared startup/pause menus have inspected CPU/GL captures; complete presentation coverage remains open. |
