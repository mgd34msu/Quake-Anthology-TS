...
Checkpoint `e20b6d6` adds ordinary Q2 rerelease roster admission and shared monster muzzle audio, including extended rerelease flash IDs. The common effects consumer now handles berserk slam and plain explosion; classic monsters emit the actor-bound typed muzzle event. Actual resource closure checks and independent roster, native source, dummy-audio and CPU effects tests passed 36 tests with 615 assertions. This source checkpoint is included in the installed `ebce976` executable.
...
Checkpoint `6331b05` rejects application launch choices whose QC, QVM or native execution path is not joined, before creating a world. This makes the existing execution boundary explicit; it does not implement a guest application path. Checkpoint `d4ed30f` preserves the modeled beam endpoint in Q2 network presentation. Root independently passed catalog and network checks (6 tests/65 assertions), including native UDP. The explicitly synthetic lower-level rerelease endpoint probe does not establish supported rerelease application wire admission or full source rendering parity. The combined eight-file snapshot passed strict TypeScript followed by policy. These checkpoints are newer than the installed `ebce976` executable.

Checkpoint `bd4308b728ee125fe60f28cb9bf40b2b6f06aa3a` preserves Q1 authored deathmatch inhibition by extending the selected-monster source mask from `0x700` to `0xf00`. Root independently passed the exact e1m5 fixture (1 test/14 assertions), covering native/selected filtering and rejection of an unknown behavior flag. The frozen source passed strict TypeScript followed by policy. The installed executable remains `ebce976`; this source correction is newer.

## Installed build

The Linux x64 executable at `/home/buzzkill/Projects/qfiles/quake-typescript` is built from checkpoint `ebce976f8d1c454d9661312f62c4de5ee3800694` (code through `0d4d3c4`), replacing the earlier `e20b6d6` binary. Running it without arguments opens the main menu before creating a world, player or game audio. Explicit CLI game/preset launches remain supported. Bun 1.3.14 produced the 119,113,856-byte executable from immutable source; strict typing, policy and input-stability gates passed. Installation was atomic with mode `0755`.

- Source SHA-256: `1d1fa5e9339c6d6dbe6d6457c3cda7114d96620113f617a16156e448b3383a69`.
- Executable SHA-256: `c45b4ae544914c85ee04ec0b2c13d45d5a0547d0c5bcf3453f77e333d5cacccc`.
- Adjacent provenance: `quake-typescript.build.json`; previous binary and sidecar: `quake-typescript.previous-20260912T160320933Z` and its `.build.json`.
- Installed help exited 0. A silent offscreen launch with no arguments remained in the menu for eight seconds, loaded no game and exited 0 on request. Root independently verified the installed hash, permissions, provenance and smoke evidence.
...
| W18/W19/W59 presentation | Checkpoint `423101a` fixes Q2 intensity, MD2 culling, Sarge child skins, audio queuing/frame cost and selected-character jump audio. | The installed executable is updated through `ebce976`; see the [build record](#installed-build). Shared startup/pause menus have inspected CPU/GL captures; complete presentation coverage remains open. |
