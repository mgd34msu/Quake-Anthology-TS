...
The implementation phase started from `6439acd6c5f0d38ca93867ae0f6100ecf4cdbee4`. The preceding source checkpoint `ec2686b` joined native server protocols, Q2 rerelease lighting/contacts and LMCTF pause/voting. Latest source checkpoint `cdc2cc5a727f6e631668c7243c2e903977c160bc` records verified snapshot `protocol-carry-CcxluG`, tree `e1b601d2f3a3a9c3e4d0b0b343c79906322a63e0`. Further edits in the live tree require their own review. W03 remains accepted only for its recorded early tooling scope. Other package states record work underway, not full acceptance. The diagram records acceptance dependencies and cannot supply a completion percentage.

## Current source and runtime state

The lead reports stable typecheck, policy and build passes for the 28-path snapshot, with 14 tests and 194 assertions. Its source fingerprint is `a3fdb83f95738c661de5f2504f62d0b11b4d256f5da9f8371ac01ac849970581`. The lead ran binary SHA-256 `950ff3ca812d28bc6935ad018cd6546dfcb5f71e62ccfeae37cf2362e8117a14` on CPU with `q1-classic-id1/e1m2`, Q1 movement and character, and on GL with `q2-classic-lmctf/lmctf09`, Q2 movement and character and two seats. Both used `listen 0` and exited with code 0. These are bounded snapshot checks, not full CPU/GL or connected-peer acceptance. This documentation refresh did not rerun gameplay.

The user's architecture rule applies to every subsystem: coalesce the strongest implementation and add every useful feature from every game to that common system. One engine, world and executable must expose those capabilities across content. Source-specific policies preserve required behavior.
...
| W41–W51 content and composition | The snapshot adds Q1 sounds/effects and frozen precaches, actual Q1 key paths and QC spatial builtins over shared raw bodies. | Full QC application lifecycle and monster builtins, exact per-map QC precache ordering, and complete composition remain unfinished. |
...
| W47 CTF/LMCTF | Pause/voting are in preceding checkpoint `ec2686b`; this snapshot adds match travel/settings. | Complete match, administration, HUD and voting workflows still require combined acceptance. |
| W57/W58 bots and navigation | Earlier application checks cover bot movement, firing, restart and travel. Machine navigation/path entities are committed; the corrected arena fixture passed 97 assertions. | The active refactor must use one Q3-derived core with useful rerelease features and normal actions under engine physics. Production admission remains Q3-only until proven. No alternate rerelease player-bot brain is intended; full navigation and team-order acceptance remain open. |
| W18/W19/W59 presentation | Q2 rerelease lighting/contacts are in `ec2686b`. The current snapshot passed the specified CPU and two-seat GL runs. | Complete presentation and workflow coverage remain open. |
...
| W52–W55 multiplayer | NetQuake 15 and Q3 protocol 68 server application joins are in `ec2686b`. This snapshot adds Q3 pure/archive downloads. | Native wire support covers family-compatible configurations. Mixed transport, connected-peer behavior and full source-wire interoperability remain unfinished. |
...
The user confirms LMCTF has no soundtrack of its own. The music check verified `lmctf09` requests track 1 and decoded inherited Q2 track 02 without changing playback or files; its warning does not establish missing LMCTF content. The main remaining chains are official gameplay → W50/W51 → W60/W66; bots/navigation → W61/W66; and W66 plus native execution → W40. W67 also requires independent evidence W63–W65, content completeness W07 and performance W68. Broad hardening follows imports, live human play and optimization under the user's code-first sequence. No full package is accepted by this refresh.
