...
The executable in `~/Projects/qfiles/quake-typescript` was built from `437bc9d` and installed on September 20. Its compiled GL check ran the saved end-map scene for 38 seconds, saved and quit cleanly; the worst measured simulation update was 24.9 ms. A separate compiled check verified stock Q3 hook attachment, pulling, saving and release, and identified the outstanding cable defect. Whole-game performance and physical audio remain unqualified. See [installed identity and runtime evidence](docs/execution-status.md#installed-executable-and-recent-fixes).
