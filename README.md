...
The executable in `~/Projects/qfiles/quake-typescript` contains production source from `67f2cfd` and was installed on September 23 (local time). It adds component source items and weapon ownership, original component cgame scenes, persistent presentation cleanup, and selected Q3/Team Arena arsenals on original id1 with pickups, equipment, saves, travel and respawn. Earlier movement, rendering, source inventory, powerup, Morning Star and end-map stall fixes remain included. Its compiled original Threewave GL check verified movement, firing, armor, save/load and clean quit. Original QC/selected-arsenal behavior has separate actual Application qualification. Whole-game performance and physical audio remain unqualified. See [installed identity and runtime evidence](docs/execution-status.md#installed-executable-and-recent-fixes).
