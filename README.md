...
QuakeC primary mods can now declare original pickup callers in `quakec-compatibility.json`. Exact artifact and instruction checks bind the recipient grant regions; the original mod retains player restrictions, feedback, map targets and respawn. This extends pickup composition beyond the built-in id1/QuakeWorld handlers. See [original pickup operations](docs/mod-compatibility.md#original-pickup-operations) for the declaration boundary.
...
Native armor mods can now declare an original inline protection region when no separate armor function exists. The mod retains its own debit and effects; nested retirement stops the owning source callback safely. This is included in installed executable `745543fb`.

Newer source also accepts artifact-qualified QuakeC weapon/client stage declarations, preserving original attack continuations and explicit objective handling through saves. Primary combat declarations also invoke the mod's complete original damage signature and armor regions; original Copper damage and save/load have been checked. Nonstock QuakeWorld programs can qualify their original damage interface through the same declaration; original Threewave QuakeWorld launch and damage passed. Modern QVM server and client modules can now declare their complete original primary interfaces, including source ammo limits and selected-weapon presentation. An original Threewave check covers selected Q2 weapons, pickups, HUD and save/load. Changed declarations reject saved continuation. Classic and rerelease native DLLs can also declare complete primary interfaces and private world layouts through [native-compatibility.json](docs/native-primary-declarations.md). Original Xatrix movement, selected Q3 firing, pickups, inventory actions and respawn passed with an external declaration; retail rerelease movement passed through its declared world layout. See [original QVM primary interfaces](docs/mod-compatibility.md#original-qvm-primary-player-interfaces).

Newer source also lets QVM combat declarations map private actor flags, damage flags, health/team slots, mass and client pointers. Complete primary declarations now support both modern and 1.16n QVM records. Input uses the original command layout; shared player updates preserve private counters and button bits. Fixed equipment poses update view height without corrupting damage events. Original calls retain their private flags through shared damage transformations. Declared QC attacker-scaling regions now drive selected Q1/Q2/Q3 weapon damage, including source modifiers active without Quad. The original region runs once, with target defenses retained. These additions are included in installed executable `c85e97aa`.

Newer source lets QuakeC, QVM and native combat declarations map their original damage, armor and reaction calls. Private names, argument order and extra inputs stay with their source. Private QVM calls use the bytecode's argument capacity instead of the public entry-point limit. Redirected hits use the current target owner, and incoming transforms run once. QC declarations can explicitly qualify an unchanged-damage region. Original QVM pickups now update component-owned items, including mutable ammo limits, and QC/QVM item counters can share capacity storage. These changes are included in installed executable `edcfada8`. Original native damage hooks also remain active without an armor component and retire cleanly with their source.

Newer source applies original QC damage operations when selected Q1/Q2/Q3 weapons and offhand grenades hit, after direct damage or splash falloff is computed. Declared pure regions can use additive arithmetic, damage-dependent branches and sequential binary32 rounding. Original Quad and other modifiers run once at impact; target defenses and explicit Q2/Q3 kick remain source-owned. This change is newer than installed `edcfada8`.

The executable in `~/Projects/qfiles/quake-typescript` contains production source from `edcfada8` and was installed on September 25 (local time). It includes the declared QC, modern/legacy QVM and native primary interfaces described above, original QC attacker scaling, native inline armor, and the selected Q3 respawn correction. The existing compiled Threewave GL check verified movement, firing, armor, save/load and normal quit; played/restored frames and saved source state were inspected. Further source contracts and composition work remain active. Whole-game performance, physical audio and complete mod coverage remain unqualified. See [installed identity and runtime evidence](docs/execution-status.md#installed-executable-and-recent-fixes).
