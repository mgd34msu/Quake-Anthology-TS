...
The executable in `~/Projects/qfiles/quake-typescript` contains production source from `af904e4` and was installed on September 22. It includes original component controls, native held-fire loops, armor composition, QVM committed-write tracking and shared water-impact trails. Its compiled original Threewave GL check verified movement, firing, armor values, save/load and clean quit. The earlier Morning Star and end-map stall fixes remain included. Whole-game performance and physical audio remain unqualified. See [installed identity and runtime evidence](docs/execution-status.md#installed-executable-and-recent-fixes).
