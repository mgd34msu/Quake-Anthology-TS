...
Native armor mods can now declare an original inline protection region when no separate armor function exists. The mod retains its own debit and effects; nested retirement stops the owning source callback safely. This is in newer source and is not yet in the installed executable.
...
The executable in `~/Projects/qfiles/quake-typescript` contains production source from `1f293c76` and was installed on September 25 (local time). It includes native selected-arsenal inventory, drops and HUDs; original equipment services; composed mod item actions and icons; additional declared QuakeC pickup callers; and reduced native memory-access allocation overhead. The existing compiled Threewave GL check verified movement, firing, armor, save/load and normal quit; three frames and saved source state were inspected. Broader original mod declarations remain in progress. Whole-game performance, physical audio and complete mod coverage remain unqualified. See [installed identity and runtime evidence](docs/execution-status.md#installed-executable-and-recent-fixes).
