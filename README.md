...
The executable in `~/Projects/qfiles/quake-typescript` contains production source from `f96d9a9` and was installed on September 22 (local time). It adds original QVM client-frame callbacks, selected base-Q3 equipment, legacy projectile save migration, persistent Team Arena powerups, and their damage/cadence effects on Q1/Q2 weapons. Earlier original pickup, native inventory, component presentation, Morning Star and end-map stall fixes remain included. Its compiled original Threewave GL check verified movement, firing, armor values, save/load and clean quit. Mixed equipment and source-weapon behavior have separate source qualification. Whole-game performance and physical audio remain unqualified. See [installed identity and runtime evidence](docs/execution-status.md#installed-executable-and-recent-fixes).
