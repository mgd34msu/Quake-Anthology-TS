...
Newer source also accepts artifact-qualified QuakeC weapon/client stage declarations, preserving original attack continuations and explicit objective handling through saves. Primary combat declarations also invoke the mod's complete original damage signature and armor regions; original Copper damage and save/load have been checked. Modern QVM server and client modules can now declare their complete original primary interfaces, including source ammo limits and selected-weapon presentation. An original Threewave check covers selected Q2 weapons, pickups, HUD and save/load. Changed declarations reject saved continuation. Native primary interface declarations remain in progress. See [original QVM primary interfaces](docs/mod-compatibility.md#original-qvm-primary-player-interfaces).
