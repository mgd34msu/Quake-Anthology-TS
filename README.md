...
QuakeC components can also declare original per-client frame calls. Retained controls drive original held-fire code; declared source think deadlines advance private weapon state and survive saves. This source change awaits executable delivery. Full source physics ordering and destination weapon-animation presentation remain separate work.
...
The executable in `~/Projects/qfiles/quake-typescript` contains production source from `8b80467` and was installed on September 22. It includes original component client sounds/effects, restored event tracking, component controls, native held-fire loops, armor composition and QVM committed-write tracking. Its compiled original Threewave GL check verified movement, firing, armor values, save/load and clean quit. The earlier Morning Star and end-map stall fixes remain included. Whole-game performance and physical audio remain unqualified. See [installed identity and runtime evidence](docs/execution-status.md#installed-executable-and-recent-fixes).
