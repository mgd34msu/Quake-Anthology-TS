...
**Options → Controls** shows numeric slider values, including overall sensitivity and horizontal/vertical multipliers. Invert mouse is separate. In the console, `bind mouse2` shows its current command and `bindlist` lists bindings. Startup control edits persist and settings scroll; earlier compiled checks covered binding-clear persistence in a second process. Installed `6103c97` includes the newer source integrations and bounded compiled mixed-gameplay, travel/save and Quit proof; see [execution status](docs/execution-status.md#installed-executable-and-recent-fixes) for the exact build and remaining limits.
