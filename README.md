...
The executable in `~/Projects/qfiles/quake-typescript` contains production source from `ffd745e` and was installed on September 22. It includes original QVM armor, HUD and save corrections, plus committed-write tracking used by mod integration. Its compiled original Threewave GL check verified movement, firing, armor values, save/load and clean quit. The earlier Morning Star and end-map stall fixes remain included. Whole-game performance and physical audio remain unqualified. See [installed identity and runtime evidence](docs/execution-status.md#installed-executable-and-recent-fixes).
