...
Newer source connects QVM components to actual player identities, userinfo changes, accepted input, and disconnects. Private source client numbers stay separate from destination players; targeted messages retain their recipient through networking and demos. Saved commands and source client state survive loading. Original LRCTF callbacks were exercised in a Q1 world with Q2 movement and Q3 Ranger. This client integration is newer than the installed executable; QuakeC and native component client adapters remain in progress.
