...
Declared QVM components connect to actual player identities, userinfo changes, accepted input, and disconnects. Private source client numbers stay separate from destination players; targeted messages retain their recipient through networking and demos. Saved commands and source client state survive loading. Original LRCTF callbacks were exercised in a Q1 world with Q2 movement and Q3 Ranger. QuakeC components now share player lifecycle, userinfo and targeted messages through explicit original callbacks, with private client state preserved in saves. Native API3/API2023 components now also use original client admission, userinfo, command and disconnect calls. Rejected clients retain their physical source row until disconnect, preventing reuse while collision still refers to it. These client integrations are included in the installed executable. Declared component callbacks on applied input are now committed in source; they await the next executable build.
...
Regular armor and powered protection now have independent state, including saves, network events and HUD values. A regular-armor change preserves powered equipment and fuel. New saves use `QTSAVE3`; version-2 saves still load, while older executables cannot read new saves. Unified peers must support snapshot version 2. Original modules retain their damage calculations and storage rules. Declared native components can supply independent powered protection using their original activation and absorption routines. One component owns the powered slot; replacing an existing owner requires an explicit declaration. Shared Q1/Q2/Q3 combat and qualified original Q2 classic/rerelease damage stages preserve the destination regular armor and health rules. Unqualified original QC/QVM damage stages reject this combination. Fuel stays in the canonical inventory, and source saves retain private activation policy. This source change awaits executable delivery.
...
The executable in `~/Projects/qfiles/quake-typescript` contains the production source of `0619295` and was installed on September 20. Its compiled GL check verified the original Morning Star, firing, three chain links, release, save/load and clean quit in a Q1 rerelease world with Q2 movement, character and weapons. Loading a held-hook save clears attack input and releases the chain. The earlier 38-second end-map check measured a worst simulation update of 24.9 ms; that fix is retained. Whole-game performance and physical audio remain unqualified. See [installed identity and runtime evidence](docs/execution-status.md#installed-executable-and-recent-fixes).
