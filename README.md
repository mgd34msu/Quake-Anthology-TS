...
Cinematic continuation now has exact decoder, movie-handle and owned-audio checkpoints, checked against original RoQ and OGV playback. Newer source also preserves original component client memory, resource handles, fonts, scripts and clocks without replaying client initialization. Connecting those client checkpoints to Application saves and cinematic transitions remains in progress. Existing component saves rebuild their client presentation when loaded.
